import { Message, Model, Part, Session, SessionStatus, SnapshotFileDiff, UserMessage } from "@aidev-cli/sdk/v2"
import { SessionTurn } from "@aidev-cli/session-ui/session-turn"
import { SessionReview } from "@aidev-cli/session-ui/session-review"
import { DataProvider } from "@aidev-cli/session-ui/context"
import { FileComponentProvider } from "@aidev-cli/ui/context/file"
import { WorkerPoolProvider } from "@aidev-cli/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@aidev-cli/ui/logo"
import { IconButton } from "@aidev-cli/ui/icon-button"
import { ProviderIcon } from "@aidev-cli/ui/provider-icon"
import { iife } from "@aidev-cli/core/util/iife"
import { Binary } from "@aidev-cli/core/util/binary"
import { NamedError } from "@aidev-cli/core/util/error"
import { DateTime } from "luxon"
import { createStore } from "solid-js/store"
import NotFound from "../[...404]"
import { Tabs } from "@aidev-cli/ui/tabs"
import { MessageNav } from "@aidev-cli/session-ui/message-nav"
import { FileSSR } from "@aidev-cli/session-ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@aidev-cli/session-ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/aidev-cli-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | AidevCLI</Title>
              </Show>
              <Meta name="description" content="aidev-cli - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              <IconButton
                                as={"a"}
                                href="https://github.com/anomalyco/aidev-cli"
                                target="_blank"
                                icon="github"
                                variant="ghost"
                              />
                              <IconButton
                                as={"a"}
                                href="https://aidev_cli.ai/discord"
                                target="_blank"
                                icon="discord"
                                variant="ghost"
                              />
