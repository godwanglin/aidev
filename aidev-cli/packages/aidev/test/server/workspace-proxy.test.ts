...
describe("HttpApi workspace proxy", () => {
  ...
  it.live("strips aidev_cli-internal headers and merges extra headers", () =>
    Effect.gen(function* () {
      ...
      expect(forwarded["x-aidev-cli-directory"]).toBeUndefined()
      expect(forwarded["x-aidev-cli-workspace"]).toBeUndefined()
