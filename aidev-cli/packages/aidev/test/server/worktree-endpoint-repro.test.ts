...
const stateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = {
      AIDEV_CLI_EXPERIMENTAL_WORKSPACES: Flag.AIDEV_CLI_EXPERIMENTAL_WORKSPACES,
    }
    ...
    yield* Effect.addFinalizer(() =>
      Effect.promise(async () => {
        Flag.AIDEV_CLI_EXPERIMENTAL_WORKSPACES = original.AIDEV_CLI_EXPERIMENTAL_WORKSPACES
        await resetDatabase()
      }),
    )
  }),
