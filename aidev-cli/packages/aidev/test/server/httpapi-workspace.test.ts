...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-aidev-cli-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-aidev-cli-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://aidev_cli.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-aidev-cli-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-aidev-cli-workspace")
