      ...
      const http = Bun.serve({
        ...
        async fetch(request) {
          ...
          if (url.pathname === "/register") {
            ...
            return Response.json({ ...metadata, client_id: "replacementClient" }, { status: 201 })
          }
...
mcpTest.instance("pending provider does not expose or overwrite existing credentials before commit", () =>
  Effect.gen(function* () {
    ...
    yield* auth.updateClientInfo(name, { clientId: "oldClient" }, url)
    ...
    expect((yield* auth.get(name))?.clientInfo?.clientId).toBe("oldClient")
  }),
)
...
mcpTest.instance("failed reauthentication preserves existing credentials", () =>
  Effect.gen(function* () {
    ...
    yield* auth.updateClientInfo(name, { clientId: "dynamicClient", clientSecret: "dynamic-secret" }, server.url)
...
mcpTest.instance("successful reauthentication commits replacement credentials", () =>
  Effect.gen(function* () {
    ...
    yield* auth.updateClientInfo(name, { clientId: "oldClient" }, server.url)
    ...
    expect(entry?.clientInfo?.clientId).toBe("replacementClient")
