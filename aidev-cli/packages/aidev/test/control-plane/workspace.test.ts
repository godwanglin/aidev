...
import { EventSequenceTable } from "@aidev-cli/core/event/sql"
...
import { WorkspaceV2 } from "@aidev-cli/core/workspace"
import { WorkspaceTable } from "@aidev-cli/core/control-plane/workspace.sql"
...
const originalEnv = {
  AIDEV_CLI_AUTH_CONTENT: process.env.AIDEV_CLI_AUTH_CONTENT,
  AIDEV_CLI_EXPERIMENTAL_WORKSPACES: process.env.AIDEV_CLI_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.AIDEV_CLI_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@aidev_cli.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.AIDEV_CLI_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=aidev_cli-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.AIDEV_CLI_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.AIDEV_CLI_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.AIDEV_CLI_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=aidev_cli-test")
