...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const aidevCliFile = path.join(tmp, ".git", "aidev-cli")
      expect(yield* Effect.promise(() => Bun.file(aidevCliFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "aidev-cli")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "aidev-cli")
      const wrongCache = path.join(parentDir, ".git", "aidev-cli")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "aidev-cli")
      const cacheB = path.join(bareB, "aidev-cli")
      const wrongCache = path.join(parentDir, ".git", "aidev-cli")
