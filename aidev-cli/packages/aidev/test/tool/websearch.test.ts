...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.AIDEV_CLI_WEBSEARCH_PROVIDER

    try {
      process.env.AIDEV_CLI_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.AIDEV_CLI_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.AIDEV_CLI_WEBSEARCH_PROVIDER
      else process.env.AIDEV_CLI_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is enabled for AidevCLI providers or explicit websearch provider flags", () => {
    expect(webSearchEnabled(ProviderV2.ID.make("aidev-cli"), { exa: false, parallel: false })).toBe(true)
    expect(webSearchEnabled(ProviderV2.ID.make("aidev-cli-go"), { exa: false, parallel: false })).toBe(true)
