...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".aidev-cli")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .aidev-cli/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const aidev_cli = path.join(test.directory, ".aidev-cli")
      const tool = path.join(aidev_cli, "tool")
  ...
  it.instance("ignores non-tool exports in .aidev-cli/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".aidev-cli", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-aidev-cli` was registering a tool
  ...
  it.instance("loads tools from .aidev-cli/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const aidev_cli = path.join(test.directory, ".aidev-cli")
      const tools = path.join(aidev_cli, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".aidev-cli", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const aidev_cli = path.join(test.directory, ".aidev-cli")
        const customTools = path.join(aidev_cli, "tools")
        const plugin = path.join(aidev_cli, "node_modules", "@aidev-cli", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(aidev_cli, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@aidev-cli/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@aidev-cli/plugin"',
              "export default tool({",
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".aidev-cli", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(aidev_cli, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@aidev-cli/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(aidev_cli, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(aidev_cli, "node_modules", "cowsay")
