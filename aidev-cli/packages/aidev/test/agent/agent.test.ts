...
it.instance("plan agent denies edits except .aidev-cli/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".aidev-cli/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".aidev-cli", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.AIDEV_CLI_TEST_HOME
      process.env.AIDEV_CLI_TEST_HOME = test.directory
