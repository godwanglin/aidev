    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".aidev-cli/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const aidevCliModel = {
      ...openaiModel,
      providerID: "aidev-cli",
      api: {
        id: "aidev-cli-test",
        url: "https://api.aidev_cli.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, aidevCliModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.aidev-cli?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.aidev-cli?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          "aidev-cli": { itemId: "msg_aidev-cli" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              "aidev-cli": { itemId: "msg_aidevCli_part" },
    ...
    expect(result[0].providerOptions?.aidev-cli?.itemId).toBe("msg_aidev-cli")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.aidev-cli?.itemId).toBe("msg_aidevCli_part")
...
describe("ProviderTransform sampling defaults - DeepSeek", () => {
  ...
  test.each([
    ...
    ["aidev-cli", "deepseek-v4-flash"],
    ["aidev-cli-go", "deepseek-v4-flash"],
