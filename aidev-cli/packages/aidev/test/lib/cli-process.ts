// Subprocess test harness for the aidev_cli CLI. Spawns the real binary against
...
//
// Configuration flows through aidev_cli's built-in test affordances:
//   - AIDEV_CLI_CONFIG_CONTENT      : provider config inline, no files to find
//   - AIDEV_CLI_TEST_HOME           : pins os.homedir() → tmpdir
//   - AIDEV_CLI_DISABLE_PROJECT_CONFIG : skip walking up for aidev_cli.json
//   - AIDEV_CLI_PURE                : skip external plugin discovery + install
//   - AIDEV_CLI_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `aidev_cli.run` is fully wired. The shape supports adding more
// builders (`aidev_cli.serve(opts)`, `aidev_cli.acp(opts)`, `aidev_cli.auth(...)`)
...
import { AppProcess } from "@"aidev-cli"/core/process"
...
const aidevCliRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(aidevCliRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    AIDEV_CLI_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    AIDEV_CLI_CONFIG_CONTENT: configJson,
    AIDEV_CLI_DISABLE_PROJECT_CONFIG: "1",
    AIDEV_CLI_PURE: "1",
    AIDEV_CLI_DISABLE_AUTOUPDATE: "1",
    AIDEV_CLI_DISABLE_AUTOCOMPACT: "1",
    AIDEV_CLI_DISABLE_MODELS_FETCH: "1",
    AIDEV_CLI_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `"aidev-cli" run`. New flags should
...
// `"aidev-cli" serve` is a long-lived process — it never exits on its own.
...
// `"aidev-cli" acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `"aidev-cli" serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `"aidev-cli" acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly ""aidev-cli"": OpencodeCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("aidev_cli.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          AIDEV_CLI_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("aidev_cli.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("aidev_cli.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   ""aidev-cli" server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `"aidev-cli" serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("aidev_cli.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const ""aidev-cli"": OpencodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, aidev_cli })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `aidev_cli.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = ""aidev-cli"") {
...
// every call site. Use this for any test that needs the aidev_cli CLI fixture.
//
...
// resources (e.g. `aidev_cli.serve`) without an extra `Effect.scoped` wrapper —
