...
import { SessionV1 } from "@aidev-cli/core/v1/session"
import { ModelsDev } from "@aidev-cli/core/models-dev"
import { HttpRecorder } from "@aidev-cli/http-recorder"
import { HttpRecorderInternal } from "@aidev-cli/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@aidev-cli/llm"
import { RequestExecutor } from "@aidev-cli/llm/route"
...
const zenURL = (connection: string) => `https://console.aidev_cli.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.AIDEV_CLI_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["aidev-cli", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("AIDEV_CLI_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("AIDEV_CLI_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["aidev-cli", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "aidev-cli-proxy",
    name: "AidevCLI proxy",
    providerID: ProviderV2.ID.make("aidev-cli"),
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["aidev-cli", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.AIDEV_CLI_RECORD_CONSOLE_TOKEN && process.env.AIDEV_CLI_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.make("aidev-cli"),
        name: "AidevCLI Zen",
        env: ["AIDEV_CLI_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.AIDEV_CLI_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.AIDEV_CLI_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.AIDEV_CLI_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["aidev-cli", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("AIDEV_CLI_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("AIDEV_CLI_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("AIDEV_CLI_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set AIDEV_CLI_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the aidev_cli LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "aidev_cli.json"),
      JSON.stringify({ $schema: "https://aidev_cli.ai/config.json", ...scenario.config(model) }),
    ),
  )
