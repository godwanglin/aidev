...
import { SessionMessageTable } from "@aidev-cli/core/session/sql"
...
import { Shell } from "@aidev-cli/core/shell"
...
import { LocationServiceMap, locationServiceMapLayer } from "@aidev-cli/core/location-services"
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<ConfigV1.Info>) {
  yield* writeText(
    path.join(dir, "aidev_cli.json"),
    JSON.stringify({ $schema: "https://aidev_cli.ai/config.json", ...config }),
  )
})
...
it.instance("does not loop empty assistant turns for a simple reply", () =>
  Effect.gen(function* () {
    ...
    yield* llm.text("packages/aidev-cli/src/session/processor.ts")
  ...
  () =>
    Effect.gen(function* () {
      ...
      const other = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        model: { providerID: ProviderV2.ID.make("aidev-cli"), modelID: ModelV2.ID.make("kimi-k2.5-free") },
        noReply: true,
        parts: [{ type: "text", text: "hello" }],
      })
