...
import { LLMClient, RequestExecutor } from "@aidev-cli/llm/route"
...
describe("session.llm.stream", () => {
  ...
  const aidevCliFixture = { providerID: "aidev-cli-test", modelID: vivgridFixture.modelID }

  it.instance(
    "sends the parent session header for aidev_cli providers",
    () =>
      Effect.gen(function* () {
        ...
        const resolved = yield* Provider.use.getModel(
          ProviderV2.ID.make(aidevCliFixture.providerID),
          ModelV2.ID.make(aidevCliFixture.modelID),
        )
        ...
        const user = {
          ...
          agent: agent.name,
          model: {
            providerID: ProviderV2.ID.make(aidevCliFixture.providerID),
            modelID: resolved.id,
          },
      ...
      }),
    {
      config: () => {
        const fixture = loadFixture(vivgridFixture.providerID, vivgridFixture.modelID)
        return {
          enabled_providers: [aidevCliFixture.providerID],
          provider: {
            [aidevCliFixture.providerID]: {
              name: "AidevCLI Test",
