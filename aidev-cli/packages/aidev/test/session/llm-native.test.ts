...
import { LLMEvent, ToolFailure } from "@aidev-cli/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@aidev-cli/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("aidev-cli") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("aidev-cli") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, aidev_cli, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored aidev_cli auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("aidev-cli") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("aidev-cli"),
