...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "aidev_cli.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "aidev_cli.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "aidev_cli.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "aidev_cli.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["aidev_cli.json", "aidev_cli.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "aidev_cli.json"),
        path.join(project, "aidev_cli.jsonc"),
      ])
    })
  })
