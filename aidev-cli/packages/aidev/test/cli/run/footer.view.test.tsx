...
import type { QuestionRequest } from "@aidev-cli/sdk/v2"
import { OpencodeKeymapProvider, registerOpencodeKeymap } from "@aidev-cli/tui/keymap"
import {
...
test("direct footer separates a lone context hint from model and command hint", async () => {
  const app = await renderFooter({
    providers: [provider()],
    currentModel: { providerID: "aidev-cli", modelID: "gpt-5" },
...
test("direct model panel renders current model selector", async () => {
  ...
  const [current] = createSignal<RunInput["model"]>({ providerID: "aidev-cli", modelID: "gpt-5" })
  ...
  try {
    ...
    expect(frame).toContain("Search")
    expect(frame).toContain("aidev-cli")
    expect(frame).toContain("GPT-5")
