// Subprocess integration tests for `aidev-cli run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `aidev_cli.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `AIDEV_CLI_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("aidev-cli run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, aidev_cli }) =>
      Effect.gen(function* () {
        ...
        const result = yield* aidev_cli.run("say hi")
        aidev_cli.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, aidev_cli }) =>
      Effect.gen(function* () {
        ...
        const result = yield* aidev_cli.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, aidev_cli }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* aidev_cli.run("think", { extraArgs: ["--thinking"] })
        aidev_cli.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* aidev_cli.run("think again")
        aidev_cli.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, aidev_cli }) =>
      Effect.gen(function* () {
        ...
        const result = yield* aidev_cli.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, aidev_cli }) =>
      Effect.gen(function* () {
        ...
        const result = yield* aidev_cli.run("say hi", { format: "json" })
        aidev_cli.expectExit(result, 0)

        const events = aidev_cli.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ aidev_cli }) =>
      Effect.gen(function* () {
        const result = yield* aidev_cli.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, aidev_cli }) =>
      Effect.gen(function* () {
        ...
        const result = yield* aidev_cli.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, aidev_cli }) =>
      Effect.gen(function* () {
        ...
        yield* llm.text("recovered")
        const result = yield* aidev_cli.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, aidev_cli }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* aidev_cli.run("request permission", { permission: { bash: "ask" } })
        aidev_cli.expectExit(denied, 0)
        ...
        const allowed = yield* aidev_cli.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        aidev_cli.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* aidev_cli.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        aidev_cli.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, aidev_cli }) =>
      Effect.gen(function* () {
        ...
        const server = yield* aidev_cli.serve()

        const result = yield* aidev_cli.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, aidev_cli }) =>
      Effect.gen(function* () {
        const result = yield* aidev_cli.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, aidev_cli }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* aidev_cli.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
