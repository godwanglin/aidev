...
// file. See script/prebuild-test_cli.ts for an opt-in pre-built binary that
// cuts per-spawn cost when this suite gets bigger.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { cliIt } from "../../lib/cli-process"

describe("aidev-cli read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // AIDEV_CLI_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ aidev_cli }) =>
      Effect.gen(function* () {
        const r = yield* aidev_cli.spawn(["mcp", "list"])
        aidev_cli.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ aidev_cli }) =>
      Effect.gen(function* () {
        const r = yield* aidev_cli.spawn(["providers", "list"])
        aidev_cli.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ aidev_cli }) =>
      Effect.gen(function* () {
        const r = yield* aidev_cli.spawn(["models"])
        aidev_cli.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ aidev_cli }) =>
      Effect.gen(function* () {
        const r = yield* aidev_cli.spawn(["agent", "list"])
        aidev_cli.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh AIDEV_CLI_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ aidev_cli }) =>
      Effect.gen(function* () {
        const r = yield* aidev_cli.spawn(["session", "list"])
        aidev_cli.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ aidev_cli }) =>
      Effect.gen(function* () {
        const r = yield* aidev_cli.spawn(["stats"])
        aidev_cli.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under AIDEV_CLI_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ aidev_cli }) =>
      Effect.gen(function* () {
        const r = yield* aidev_cli.spawn(["db", "path"])
        aidev_cli.expectExit(r, 0, "db path")
