...
import { expectOk, selectConfigOption } from "./acp-testClient"
import { createAcpClient, initialize, newSession, verifierConfig } from "./helpers"

describe("aidev-cli acp lifecycle subprocess", () => {
  cliIt.live(
    "stdin EOF exits cleanly",
    ({ aidev_cli }) =>
      Effect.gen(function* () {
        const acp = yield* aidev_cli.acp()
        acp.close()
    ...
    ({ home, llm, aidev_cli }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient(
          { aidev_cli },
          { AIDEV_CLI_CONFIG_CONTENT: JSON.stringify(verifierConfig(llm.url)) },
        )
