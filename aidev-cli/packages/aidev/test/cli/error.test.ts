...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/aidev_cli.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/aidev_cli.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/aidev_cli.jsonc", dir: ".aidev-cli", suggestion: "aidev-cli" },
        expected:
          'Directory ".aidev-cli" in /tmp/aidev_cli.jsonc is not valid. Rename the directory to "aidev-cli" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/aidev_cli.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/aidev_cli.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats account transport errors clearly", () => {
    const error = new AccountTransportError({
      method: "POST",
      url: "https://console.aidev_cli.ai/auth/device/code",
    })

    const formatted = FormatError(error)

    expect(formatted).toContain("Could not reach POST https://console.aidev_cli.ai/auth/device/code.")
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `aidev-cli models` to list available models",
      "Or check your config (aidev_cli.json) provider/model names",
    ].join("\n")
