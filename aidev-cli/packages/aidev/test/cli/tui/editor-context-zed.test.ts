...
import { isZedTerminal, offsetToPosition, resolveZedDbPath, resolveZedSelection } from "@aidev-cli/tui/editor-zed"
...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const previous = process.env.AIDEV_CLI_ZED_DB
  process.env.AIDEV_CLI_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) delete process.env.AIDEV_CLI_ZED_DB
    else process.env.AIDEV_CLI_ZED_DB = previous
    home.mockRestore()
  }
})
