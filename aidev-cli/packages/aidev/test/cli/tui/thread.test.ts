...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ aidev_cli }) =>
    Effect.gen(function* () {
      const result = yield* aidev_cli.spawn(["--replay-limit", "10"])

      aidev_cli.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ aidev_cli }) =>
    Effect.gen(function* () {
      const result = yield* aidev_cli.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ aidev_cli }) =>
    Effect.gen(function* () {
      const result = yield* aidev_cli.spawn(["--mini", "--port", "4096"])
