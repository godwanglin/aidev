...
// terminal sizes. The default aidev_cli tui command is excluded —
// `aidev-cli --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `aidev-cli --help` prints today;
...
// top-level help on `--help` and exits 1; not a real aidev_cli command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("aidev-cli CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ aidev_cli }) =>
      Effect.gen(function* () {
        const topLevel = yield* aidev_cli.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* aidev_cli.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`aidev-cli ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`aidev-cli ${argv.join(" ")} --help`)
        }
