...
- Running `bun dev` from `packages/aidev-cli` starts the live interactive TUI. Do not run it as a blocking foreground command when you need to inspect the result.
- Start it in `tmux` instead: `tmux new-session -d -s aidev-cli-dev 'bun dev'`.
- Capture the current TUI output with: `tmux capture-pane -pt aidev-cli-dev`.
- Stop the session explicitly when done: `tmux kill-session -t aidev-cli-dev`.
...
export class Service extends Context.Service<Service, Interface>()("@aidev-cli/Foo") {}
