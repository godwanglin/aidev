...
export const DEPRECATED_PLUGIN_PACKAGES = ["aidev-cli-openai-codex-auth", "aidev-cli-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, aidevCliVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(aidevCliVersion) || semver.major(aidevCliVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.aidev-cli
  if (typeof range !== "string") return
  if (!semver.satisfies(aidevCliVersion, range)) {
    throw new Error(`Plugin requires aidev_cli ${range} but running ${aidevCliVersion}`)
  }
}
