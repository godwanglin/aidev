...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@aidev-cli/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@aidev-cli/core/effect/app-node-builder"
import { errorData, errorMessage } from "@aidev-cli/tui/util/error"
...
import { hasTheme, upsertTheme } from "@aidev-cli/tui/context/theme"
import { Global } from "@aidev-cli/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@aidev-cli/core/util/flock"
import { Flag } from "@aidev-cli/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@aidev-cli/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@aidev-cli/core/v1/config/plugin"
import { createCommandShim } from "@aidev-cli/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@aidev-cli/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".aidev-cli"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".aidev-cli", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".aidev-cli", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.AIDEV_CLI_PURE ? [] : pluginOrigins
    if (Flag.AIDEV_CLI_PURE && pluginOrigins.length) {
    }
