...
import type { AssistantMessage, Message, OpencodeClient, SessionMessageResponse } from "@aidev-cli/sdk/v2"
...
export const AuthMethodID = "aidev-cli-login"
...
export class Service extends Context.Service<Service, Interface>()("@aidev-cli/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `aidev-cli auth login` in the terminal",
      name: "Login with aidev_cli",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, aidev_cli provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const aidevCliProvider = providers[ProviderV2.ID.make("aidev-cli")]
  const aidevCliModel = aidevCliProvider ? Provider.sort(Object.values(aidevCliProvider.models))[0] : undefined
  if (aidevCliProvider && aidevCliModel) return { providerID: aidevCliProvider.id, modelID: aidevCliModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "AidevCLI service failure", service })
}
