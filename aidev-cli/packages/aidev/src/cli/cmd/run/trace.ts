...
//
// Enable with AIDEV_CLI_DIRECT_TRACE=1. Writes one JSON line per event to
// ~/.local/share/aidev-cli/log/direct/<timestamp>-<pid>.jsonl. Also writes
...
export function trace(): Trace | undefined {
  ...
  if (!process.env.AIDEV_CLI_DIRECT_TRACE) {
    state = false
    return undefined
  }
