import type { PermissionV1 } from "@aidev-cli/core/v1/permission"
import { FSUtil } from "@aidev-cli/core/fs-util"
// CLI entry point for `aidev-cli run` and `aidev-cli --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`aidev-cli --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`aidev-cli --mini --attach`): connects to a running
//      aidev_cli server and runs interactive mode against it.
//
...
import { createAidevClient, type OpencodeClient, type ToolPart } from "@aidev-cli/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run aidev_cli with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running aidev_cli server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to AIDEV_CLI_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to AIDEV_CLI_SERVER_USERNAME or 'aidev-cli')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const sdk = createAidevClient({
        baseUrl: "http://aidev_cli.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
