...
import { AidevCLIHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for aidev_cli serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(AidevCLIHttpApi)
...
export const PublicApi = AidevCLIHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "aidev-cli",
    version: "1.0.0",
    description: "aidev-cli api",
