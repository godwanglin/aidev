...
import { serviceUse } from "@aidev-cli/core/effect/service-use"
...
import { InstallationLocal, InstallationVersion } from "@aidev-cli/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@aidev-cli/core/v1/config/console-state"
import { FSUtil } from "@aidev-cli/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@aidev-cli/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@aidev-cli/core/v1/config/config"
import { RemoteAuthError } from "@aidev-cli/core/v1/config/error"
import { ConfigPermissionV1 } from "@aidev-cli/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@aidev-cli/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["aidev_cli.jsonc", "aidev_cli.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://aidev_cli.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.AIDEV_CLI_CONFIG && !Flag.AIDEV_CLI_CONFIG_DIR && !Flag.AIDEV_CLI_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://aidev_cli.ai/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "aidev_cli.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "aidev_cli.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://aidev_cli.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "AIDEV_CLI_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/aidev-cli`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://aidev_cli.ai/config.json"
        ...
        if (Flag.AIDEV_CLI_CONFIG) {
          yield* merge(Flag.AIDEV_CLI_CONFIG, yield* loadFile(Flag.AIDEV_CLI_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.AIDEV_CLI_CONFIG })
        }

        if (!Flag.AIDEV_CLI_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("aidev-cli", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.AIDEV_CLI_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from AIDEV_CLI_CONFIG_DIR", { path: Flag.AIDEV_CLI_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".aidev-cli") || dir === Flag.AIDEV_CLI_CONFIG_DIR) {
            for (const file of ["aidev_cli.json", "aidev_cli.jsonc"]) {
          ...
          // Auto-discovered plugins under `.aidev-cli/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.AIDEV_CLI_CONFIG_CONTENT) {
          const source = "AIDEV_CLI_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.AIDEV_CLI_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from AIDEV_CLI_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["AIDEV_CLI_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("AIDEV_CLI_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.AIDEV_CLI_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.AIDEV_CLI_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("AIDEV_CLI_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.AIDEV_CLI_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.AIDEV_CLI_DISABLE_PRUNE) {
