...
import { NonNegativeInt, PositiveInt } from "@aidev-cli/core/schema"
import { ConfigAttachmentV1 } from "@aidev-cli/core/v1/config/attachment"
import { ConfigLSPV1 } from "@aidev-cli/core/v1/config/lsp"
import { InvalidError } from "@aidev-cli/core/v1/config/error"
  ...
  if (permissions.length)
    throw new InvalidError({
      ...
      issues: permissions.map((path) => ({
        path,
        message: 'V2 permissions are not supported by AidevCLI V1. Use V1 "permission" rules or run aidevCli2.',
      })),
    })
