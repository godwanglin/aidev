...
const TUI_SCHEMA_URL = "https://aidev_cli.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from aidev_cli.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const aidev_cli = await aidevCliFiles(input)
  for (const file of aidev_cli) {
...
async function aidevCliFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "aidev-cli"),
    ...(await Filesystem.findUp(["aidev_cli.json", "aidev_cli.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "aidev-cli"))
  }
  if (Flag.AIDEV_CLI_CONFIG) files.push(Flag.AIDEV_CLI_CONFIG)
