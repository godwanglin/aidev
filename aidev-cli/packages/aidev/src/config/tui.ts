...
import { isRecord } from "@aidev-cli/tui/util/record"
...
import { TuiKeybind } from "@aidev-cli/tui/config/keybind"
...
export class Service extends Context.Service<Service, Interface>()("@aidev-cli/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old aidev_cli.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.aidev-cli`
  // folders between cwd and home, and AIDEV_CLI_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.AIDEV_CLI_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit AIDEV_CLI_TUI_CONFIG override, if set.
  if (Flag.AIDEV_CLI_TUI_CONFIG) {
    const configFile = Flag.AIDEV_CLI_TUI_CONFIG
  ...
  // 4. `.aidev-cli` directories (and AIDEV_CLI_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".aidev-cli") || dir === Flag.AIDEV_CLI_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".aidev-cli") && dir !== Flag.AIDEV_CLI_CONFIG_DIR) continue
