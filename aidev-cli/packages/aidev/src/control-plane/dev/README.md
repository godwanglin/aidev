This is a plugin to simulate a remote environment locally. Add this to `.aidev-cli/aidev-cli.jsonc`:

```json
  "plugin": ["../packages/aidev-cli/src/control-plane/dev/debug-workspace-plugin.ts"],
```

In a separate terminal, run a separate AidevCLI server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/aidev-cli/script/run-workspace-server
```

With the plugin install, you can now run AidevCLI and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
