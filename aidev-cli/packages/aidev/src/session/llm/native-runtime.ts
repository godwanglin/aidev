...
import type { LLMClientShape } from "@aidev-cli/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("aidev-cli"))
    return { type: "unsupported", reason: "provider is not openai, aidev_cli, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @aidev-cli/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains aidev_cli-owned. The native runtime only adapts
      // the @aidev-cli/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
