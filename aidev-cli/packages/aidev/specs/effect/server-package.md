...
Practical reference for a future `packages/server` split after the aidev-cli
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/aidev-cli`.
...
Until enough shared service code lives outside `packages/aidev-cli`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/aidev-cli`

It should not import `packages/aidev-cli` services while `packages/aidev-cli`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on aidev-cli-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/aidev-cli` implementation details.
