...
Delete `packages/aidev-cli/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/aidev-cli/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/aidev-cli/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/aidev-cli`
- TUI plugin loader tests in `packages/aidev-cli` if runtime plugin API wiring changed
