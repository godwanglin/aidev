...
- Author package entrypoint is `@aidev-cli/plugin/tui`.
...
```json
{
  "$schema": "https://aidev-cli.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/aidev-cli-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@aidev-cli/plugin/tui`.
- `@aidev-cli/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@aidev-cli/plugin/tui"
...
- The value is a semver range checked against the running AidevCLI version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.aidev-cli` is absent, no check is performed (backward compatible).
...
- `aidev-cli plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `aidev-cli plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.aidev-cli` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.aidev-cli`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.aidev-cli`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.aidev-cli` dir or `AIDEV_CLI_CONFIG_DIR`, AidevCLI installs `@aidev-cli/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@aidev-cli/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus AidevCLI's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
AidevCLI registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, AidevCLI pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@aidev-cli/plugin/tui`.
...
- `message` is required; `title` defaults to `"aidev-cli"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `aidev-cli.default`.
...
- Local plugins persist installed themes under the local `.aidev-cli/themes` area near the plugin config source.
...
- `--pure` / `AIDEV_CLI_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.aidev-cli/plugins/tui-smoke.tsx`
- Local vim plugin: `.aidev-cli/plugins/tui-vim.tsx`
- Local smoke config: `.aidev-cli/tui.json`
- Local smoke theme: `.aidev-cli/plugins/smoke-theme.json`
