---
title: TUI
description: Using the AidevCLI terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

AidevCLI provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running AidevCLI starts the TUI for the current directory.
...
When using the AidevCLI TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to AidevCLI. Allows you to select from available providers and add their API keys.
...
Exit AidevCLI. _Aliases_: `/quit`, `/q`
...
This is separate from `aidev-cli.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long AidevCLI waits after the leader key. Defaults to `2000`.
...
Use `AIDEV_CLI_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `aidev-cli.default`.
