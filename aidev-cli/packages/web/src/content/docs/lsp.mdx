---
title: LSP Servers
description: AidevCLI integrates with your LSP servers.
---

AidevCLI can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

AidevCLI comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `AIDEV_CLI_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and aidev-cli opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your aidev-cli config.
...
```json title="aidev-cli.json" {5-13}
{
  "$schema": "https://aidev-cli.ai/config.json",
  "lsp": {
    "custom-lsp": {
