...
You can add external tools to AidevCLI using the _Model Context Protocol_, or MCP. AidevCLI supports both local and remote servers.
...
You can define MCP servers in your [AidevCLI Config](https://aidev-cli.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/aidev-cli` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
AidevCLI automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, AidevCLI will:
...
If the server requires authentication, AidevCLI will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `aidev-cli mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, AidevCLI will store the tokens securely in `~/.local/share/aidev-cli/mcp-auth.json`.
...
Your MCPs are available as tools in AidevCLI, alongside built-in tools. So you can manage them through the AidevCLI config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect AidevCLI to your Sentry account.
