...
With AidevCLI you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, AidevCLI uses our own `aidev-cli` theme.
...
AidevCLI comes with several built-in themes.
...
- Want AidevCLI to match their terminal's appearance
...
AidevCLI supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/aidev-cli/themes/*.json` or `$XDG_CONFIG_HOME/aidev-cli/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.aidev-cli/themes/*.json`
4. **Current working directory** - Defined in `./.aidev-cli/themes/*.json`
