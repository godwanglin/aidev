---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć aidev-cli.
---

Wtyczki umożliwiające wyłączenie aidev-cli poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania aidev-cli.
...
- `.aidev-cli/plugins/` - Wtyczki na poziomie projektu
- `~/.config/aidev-cli/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/aidev-cli/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/aidev-cli/aidev-cli.json`)
2. Konfiguracja projektu (`aidev-cli.json`)
3. Globalny katalog wtyczek (`~/.config/aidev-cli/plugins/`)
4. Katalog wtyczek projektu (`.aidev-cli/plugins/`)
...
aidev-cli uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z aidev-cli do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do aidev-cli.
...
:::note
Jeśli korzystasz z aplikacji komputerowej aidev-cli, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj aidev-cli czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do aidev-cli:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod aidev-cli. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://aidev-cli.ai/docs/sdk).
