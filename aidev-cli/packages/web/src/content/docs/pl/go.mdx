...
AidevCLI Go to niskokosztowa subskrypcja za **10 $/miesiąc**, która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

Go działa jak każdy inny dostawca w AidevCLI. Subskrybujesz AidevCLI Go i
...
AidevCLI Go daje Ci dostęp do tych modeli za **10 $/miesiąc**.

---

## Jak to działa

AidevCLI Go działa jak każdy inny dostawca w AidevCLI.

1. Logujesz się do **<a href={console}>AidevCLI Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `AidevCLI Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować AidevCLI Go.
:::
...
AidevCLI Go jest przeznaczony do użytku z [AidevCLI](https://aidev-cli.ai) i innymi popularnymi agentami kodującymi, którzy generują podobne rodzaje żądań.

Ruch jest monitorowany pod kątem nadużyć, które pogarszają komfort korzystania z usługi przez innych użytkowników.

Aby Twoje konto nie zostało oznaczone, upewnij się, że używane przez Ciebie narzędzie

1\. nie generuje ruchu stanowiącego nadużycie  
2\. prawidłowo się identyfikuje (bez ogólnych identyfikatorów User-Agent)  
3\. zawiera nagłówek `x-aidev-cli-session`, abyśmy mogli zoptymalizować buforowanie promptów

## Limity użycia

AidevCLI Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji AidevCLI
używa formatu `aidev-cli-go/<model-id>`. Na przykład dla Kimi K3 należy użyć
`aidev-cli-go/kimi-k3` w swojej konfiguracji.
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z AidevCLI.
