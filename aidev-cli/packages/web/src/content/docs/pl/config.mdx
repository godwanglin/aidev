---
title: Konfiguracja
description: Ustawienia AidevCLI w pliku JSON.
---

Możesz dostosować AidevCLI za pomocą pliku konfiguracyjnego JSON.

---

## Format

AidevCLI obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "aidev-cli"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/aidev-cli`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/aidev-cli/aidev-cli.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `AIDEV_CLI_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`aidev-cli.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.aidev-cli`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `AIDEV_CLI_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.aidev-cli` i `~/.config/aidev-cli` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/aidev-cli`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację AidevCLI w `~/.config/aidev-cli/aidev-cli.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/aidev-cli/tui.json`.
...
Dodaj `aidev-cli.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy AidevCLI się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `AIDEV_CLI_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `AIDEV_CLI_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.aidev-cli` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.aidev-cli`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`aidev-cli.ai/config.json`**](https://aidev-cli.ai/config.json).
...
Użyj `AIDEV_CLI_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `aidev-cli.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `aidev-cli serve` i `aidev-cli web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer AidevCLI.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `aidev-cli.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie AidevCLI wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/aidev-cli/agents/` lub `.aidev-cli/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, AidevCLI powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`aidev-cli run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/aidev-cli/commands/` lub `.aidev-cli/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
AidevCLI może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy AidevCLI nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie AidevCLI **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają AidevCLI o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.aidev-cli/plugins/` lub `~/.config/aidev-cli/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie AidevCLI tylko do określonych dostawców.
