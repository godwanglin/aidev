---
title: Serwery LSP
description: AidevCLI integruje się z serwerami LSP.
---

AidevCLI może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

AidevCLI posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `AIDEV_CLI_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i AidevCLI otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji AidevCLI.
