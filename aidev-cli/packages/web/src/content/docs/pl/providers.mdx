---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w aidev-cli.
---

import config from "../../../../config.mjs"
export const console = config.console

aidev-cli używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji aidev-cli.
...
w `~/.local/share/aidev-cli/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim aidev-cli
config.
...
AidevCLI Zen to lista modeli dostarczonych przez zespół aidev-cli, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z aidev-cli. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od AidevCLI Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `AidevCLI Zen` i przejdź do [aidev-cli.ai/auth](https://aidev-cli.ai/zen).
...
Działa jak każdy inny dostawca w aidev-cli i jest całkowicie opcjonalny w użyciu.

---

## AidevCLI Go

AidevCLI Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół aidev-cli, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z aidev-cli.

1. Uruchom polecenie `/connect` w TUI, wybierz `AidevCLI Go` i przejdź do [aidev-cli.ai/auth](https://aidev-cli.ai/zen).
...
Aby używać Amazon Bedrock z aidev-cli:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania aidev-cli:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `aidev-cli.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w aidev-cli nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować aidev-cli do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod aidev-cli działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji aidev-cli.
...
aidev-cli używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować aidev-cli
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`aidev-cli.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [aidev-cli-gitlab-auth](https://www.npmjs.com/package/aidev-cli-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem aidev-cli:
...
Aby używać Google Vertex AI z aidev-cli:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania aidev-cli.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą aidev-cli, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [aidev-cli-helicone-session](https://github.com/H2Shami/aidev-cli-helicone-session), aby automatycznie rejestrować każdą konwersację aidev-cli jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja aidev-cli będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować aidev-cli tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować aidev-cli tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować aidev-cli tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem aidev-cli. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/aidev-cli).
:::
...
Aby korzystać z Ollama Cloud z aidev-cli:
...
3. Skopiuj klucz API do użycia w aidev-cli.
...
6. **Ważne**: Przed użyciem modeli chmurowych w aidev-cli musisz pobrać informacje o modelu lokalnie:
...
AidevCLI Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół aidev-cli. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>AidevCLI Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **AidevCLI Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API aidev-cli.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji aidev-cli.
...
5. Możesz je także dostosować za pomocą konfiguracji aidev-cli. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z aidev-cli:
...
Możesz także dostosować modele za pomocą konfiguracji aidev-cli. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z aidev-cli. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `aidev-cli.json` w katalogu projektu:
...
Pola `limit` pozwalają aidev-cli zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `aidev-cli auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację aidev-cli i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji aidev-cli.
