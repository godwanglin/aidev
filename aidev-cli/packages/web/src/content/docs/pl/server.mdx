---
title: Serwer
description: Interakcja z serwerem aidev-cli za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `aidev-cli serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient aidev-cli.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `aidev-cli.local` |
...
Ustaw `AIDEV_CLI_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `aidev-cli` lub ustaw `AIDEV_CLI_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `aidev-cli serve`, jak i `aidev-cli web`.
...
Kiedy uruchomisz `aidev-cli`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera aidev-cli do programowej interakcji z aidev-cli.
:::

Ta architektura umożliwia obsługę wielu klientów przez aidev-cli i programową interakcję z kodem otwartym.

Możesz uruchomić `aidev-cli serve`, aby uruchomić samodzielny serwer. Jeśli masz
aidev-cli TUI działa, `aidev-cli serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki aidev-cli [IDE](/docs/ide).
...
Serwer aidev-cli udostępnia następujące interfejsy API.
