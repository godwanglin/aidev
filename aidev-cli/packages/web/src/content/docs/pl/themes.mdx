...
Dzięki aidev-cli możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie aidev-cli używa naszego własnego motywu `aidev-cli`.
...
aidev-cli ma kilka wbudowanych motywów.
...
- Chcą, aby AidevCLI pasował do wyglądu ich terminala
...
aidev-cli obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/aidev-cli/themes/*.json` lub `$XDG_CONFIG_HOME/aidev-cli/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.aidev-cli/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.aidev-cli/themes/*.json`
