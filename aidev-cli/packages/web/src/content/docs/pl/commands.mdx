...
Utwórz `.aidev-cli/commands/test.md`:
...
Możesz zdefiniować polecenia w `aidev-cli.json` lub w plikach w katalogu `commands/`.

---

### JSON

Użyj opcji `command` w swojej konfiguracji aidev-cli:
...
- Globalnie: `~/.config/aidev-cli/commands/`
- Na projekt: `.aidev-cli/commands/`
...
aidev-cli zawiera kilka wbudowanych poleceń, takich jak `/init`, `/undo`, `/redo`, `/share`, `/help`; [dowiedz się więcej](/docs/tui#commands).
