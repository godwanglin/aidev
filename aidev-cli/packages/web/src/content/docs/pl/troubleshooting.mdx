...
Aby debugować problemy z aidev-cli, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\aidev-cli\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `aidev-cli --log-level DEBUG`.

---

## Przechowywanie danych

aidev-cli przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/aidev-cli/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\aidev-cli`
...
aidev-cli Desktop uruchamia w tle lokalny serwer aidev-cli (sidecar `aidev-cli-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `AidevCLI` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/aidev-cli/aidev-cli.jsonc` (lub `~/.config/aidev-cli/aidev-cli.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/aidev-cli/aidev-cli.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\aidev-cli\aidev-cli.jsonc`
...
aidev-cli może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/aidev-cli/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\aidev-cli\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby aidev-cli mógł ją odbudować.

1. Całkowicie zamknij aidev-cli Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/aidev-cli`
- **Linux**: usuń `~/.cache/aidev-cli` (lub uruchom `rm -rf ~/.cache/aidev-cli`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\aidev-cli`

3. Uruchom ponownie aidev-cli Desktop.

---

### Napraw problemy z połączeniem z serwerem

aidev-cli Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `aidev-cli.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `AIDEV_CLI_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `AIDEV_CLI_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows aidev-cli Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami aidev-cli.

---

### Brak powiadomień

aidev-cli Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla aidev-cli w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie aidev-cli Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji aidev-cli Desktop):

- `aidev-cli.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `aidev-cli.global.dat` i `aidev-cli.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z aidev-cli:
...
3. Upewnij się, że masz najnowszą wersję z `aidev-cli upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `aidev-cli models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\aidev-cli`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. aidev-cli dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\aidev-cli`

2. Uruchom ponownie kod aidev-cli, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to aidev-cli do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
aidev-cli wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
