...
Umiejętności agenta pozwalają aidev-cli odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
aidev-cli przeszukuje te lokalizacje:

- Project config: `.aidev-cli/skills/<name>/SKILL.md`
- Global config: `~/.config/aidev-cli/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu aidev-cli przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.aidev-cli/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/aidev-cli/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.aidev-cli/skills/git-release/SKILL.md` w ten sposób:
...
aidev-cli wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `aidev-cli.json`:
...
**Dla agentów wbudowanych** (w `aidev-cli.json`):
