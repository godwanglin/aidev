...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. AidevCLI posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w AidevCLI.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `AIDEV_CLI_EXPERIMENTAL_LSP_TOOL=true` (lub `AIDEV_CLI_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy AidevCLI lub gdy zmienna środowiskowa `AIDEV_CLI_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu AidevCLI:
