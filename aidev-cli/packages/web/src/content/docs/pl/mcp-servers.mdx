...
Możesz dodać dodatkowe informacje do aidev-cli za pomocą _Model Context Protocol_, w skrócie MCP. aidev-cli obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [aidev-cli Config](https://aidev-cli.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/aidev-cli`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
aidev-cli automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, aidev-cli:
...
Jeśli serwer wymaga uwierzytelnienia, aidev-cli poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `aidev-cli mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji aidev-cli bezpieczne przechowa tokeny w `~/.local/share/aidev-cli/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w aidev-cli, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie aidev-cli, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć aidev-cli z kontem Sentry.
