---
title: Configuração
description: Usando a configuração JSON do aidev-cli.
---

Você pode configurar o aidev-cli usando um arquivo de configuração JSON.

---

## Formato

O aidev-cli suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
1. **Configuração remota** (de `.well-known/aidev-cli`) - padrões organizacionais
2. **Configuração global** (`~/.config/aidev-cli/aidev-cli.json`) - preferências do usuário
3. **Configuração personalizada** (`AIDEV_CLI_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`aidev-cli.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.aidev-cli`** - agentes, comandos, plugins
6. **Configuração inline** (`AIDEV_CLI_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.aidev-cli` e `~/.config/aidev-cli` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
As organizações podem fornecer configuração padrão através do endpoint `.well-known/aidev-cli`. Isso é buscado automaticamente quando você se autentica com um provedor que o suporta.
...
Coloque sua configuração global do aidev-cli em `~/.config/aidev-cli/aidev-cli.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/aidev-cli/tui.json`.
...
Adicione `aidev-cli.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Quando o aidev-cli é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `AIDEV_CLI_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `AIDEV_CLI_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.aidev-cli`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.aidev-cli`, então ele **pode substituir** suas configurações.

---

## Esquema

O esquema de configuração do servidor/tempo de execução é definido em [**`aidev-cli.ai/config.json`**](https://aidev-cli.ai/config.json).

A configuração do TUI usa [**`aidev-cli.ai/tui.json`**](https://aidev-cli.ai/tui.json).
...
Use `AIDEV_CLI_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.

Chaves legadas `theme`, `keybinds` e `tui` em `aidev-cli.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você pode configurar as configurações do servidor para os comandos `aidev-cli serve` e `aidev-cli web` através da opção `server`.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor aidev-cli.
- `mdnsDomain` - Nome de domínio personalizado para o serviço mDNS. O padrão é `aidev-cli.local`. Útil para executar várias instâncias na mesma rede.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do aidev-cli através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o aidev-cli tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/aidev-cli/agents/` ou `.aidev-cli/agents/`. [Saiba mais aqui](/docs/agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o aidev-cli recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`aidev-cli run`), aplicativo desktop e GitHub Action.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/aidev-cli/commands/` ou `.aidev-cli/commands/`. [Saiba mais aqui](/docs/commands).
...
O aidev-cli fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o aidev-cli **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o aidev-cli com ferramentas, hooks e integrações personalizadas.

Coloque arquivos de plugin em `.aidev-cli/plugins/` ou `~/.config/aidev-cli/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
...
Isso é útil quando você deseja restringir o aidev-cli para usar apenas provedores específicos, em vez de desabilitá-los um a um.
