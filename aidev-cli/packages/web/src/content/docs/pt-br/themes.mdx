...
Com o aidev-cli, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o aidev-cli usa nosso próprio tema `aidev-cli`.
...
O aidev-cli vem com vários temas embutidos.
...
- Querem que o aidev-cli corresponda à aparência do seu terminal
...
O aidev-cli suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/aidev-cli/themes/*.json` ou `$XDG_CONFIG_HOME/aidev-cli/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.aidev-cli/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.aidev-cli/themes/*.json`
