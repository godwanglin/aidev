...
As habilidades do agente permitem que o aidev-cli descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O aidev-cli pesquisa nesses locais:

- Configuração do projeto: `.aidev-cli/skills/<name>/SKILL.md`
- Configuração global: `~/.config/aidev-cli/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o aidev-cli sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.aidev-cli/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/aidev-cli/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.aidev-cli/skills/git-release/SKILL.md` assim:
...
O aidev-cli lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `aidev-cli.json`:
...
**Para agentes embutidos** (em `aidev-cli.json`):
