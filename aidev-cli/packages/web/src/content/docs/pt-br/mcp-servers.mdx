...
Você pode adicionar ferramentas externas ao aidev-cli usando o _Model Context Protocol_, ou MCP. O aidev-cli suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do aidev-cli](https://aidev-cli.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/aidev-cli`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O aidev-cli lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o aidev-cli irá:
...
Se o servidor requer autenticação, o aidev-cli solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `aidev-cli mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o aidev-cli armazenará os tokens de forma segura em `~/.local/share/aidev-cli/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no aidev-cli, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do aidev-cli como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o aidev-cli à sua conta do Sentry.
