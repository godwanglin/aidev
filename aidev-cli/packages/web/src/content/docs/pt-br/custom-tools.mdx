...
description: Crie ferramentas que o LLM pode chamar no aidev-cli.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do aidev-cli, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.aidev-cli/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/aidev-cli/tools/`.
