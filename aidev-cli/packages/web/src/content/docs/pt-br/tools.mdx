...
As ferramentas permitem que o LLM execute ações em sua base de código. O aidev-cli vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no aidev-cli.
...
:::note
Esta ferramenta está disponível apenas quando `AIDEV_CLI_EXPERIMENTAL_LSP_TOOL=true` (ou `AIDEV_CLI_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor aidev-cli ou quando a variável de ambiente `AIDEV_CLI_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o aidev-cli:
