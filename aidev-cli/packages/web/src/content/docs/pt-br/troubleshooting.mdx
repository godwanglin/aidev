...
Para depurar problemas com o aidev-cli, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\aidev-cli\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `aidev-cli --log-level DEBUG`.

---

## Armazenamento

O aidev-cli armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/aidev-cli/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\aidev-cli`
...
O aidev-cli Desktop executa um servidor aidev-cli local (o sidecar `aidev-cli-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `AidevCLI` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/aidev-cli/aidev-cli.jsonc` (ou `~/.config/aidev-cli/aidev-cli.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/aidev-cli/aidev-cli.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\aidev-cli\aidev-cli.jsonc`
...
O aidev-cli também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/aidev-cli/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\aidev-cli\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o aidev-cli possa reconstruí-lo.

1. Saia completamente do aidev-cli Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/aidev-cli`
- **Linux**: exclua `~/.cache/aidev-cli` (ou execute `rm -rf ~/.cache/aidev-cli`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\aidev-cli`

3. Reinicie o aidev-cli Desktop.

---

### Corrigindo problemas de conexão com o servidor

O aidev-cli Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `aidev-cli.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `AIDEV_CLI_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `AIDEV_CLI_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o aidev-cli Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do aidev-cli.

---

### Notificações não aparecendo

O aidev-cli Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o aidev-cli nas configurações do seu sistema operacional, e
...
1. Saia do aidev-cli Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo aidev-cli Desktop):

- `aidev-cli.settings.dat` (URL do servidor padrão do desktop)
- `aidev-cli.global.dat` e `aidev-cli.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o aidev-cli:
...
Para descobrir quais modelos você tem acesso, execute `aidev-cli models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\aidev-cli`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O aidev-cli instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\aidev-cli`

2. Reinicie o aidev-cli para reinstalar os pacotes de provedores mais recentes

Isso forçará o aidev-cli a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O aidev-cli detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
