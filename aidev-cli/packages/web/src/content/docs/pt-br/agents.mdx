...
Existem dois tipos de agentes no aidev-cli; agentes primários e subagentes.
...
aidev-cli vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
AidevCLI vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

AidevCLI vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do AidevCLI, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `aidev-cli.json`:
...
- Global: `~/.config/aidev-cli/agents/`
- Por projeto: `.aidev-cli/agents/`

```markdown title="~/.config/aidev-cli/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o aidev-cli usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do aidev-cli quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do aidev-cli usa o formato `provider/model-id`. Por exemplo, se você estiver usando [AidevCLI Zen](/docs/zen), você usaria `aidev-cli/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="aidev-cli.json" {3-6,9-12}
{
  "$schema": "https://aidev-cli.ai/config.json",
  "tools": {
    "write": true,
...
```json title="aidev-cli.json" {3-5,8-10}
{
  "$schema": "https://aidev-cli.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `aidev-cli models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/aidev-cli).
:::
...
```markdown title="~/.config/aidev-cli/agents/docs-writer.md"
---
...
```markdown title="~/.config/aidev-cli/agents/security-auditor.md"
---
