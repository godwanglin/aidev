---
title: Plugins
description: Escreva seus próprios plugins para estender o aidev-cli.
---

Plugins permitem que você estenda o aidev-cli conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do aidev-cli.
...
- `.aidev-cli/plugins/` - Plugins em nível de projeto
- `~/.config/aidev-cli/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/aidev-cli/node_modules/`.
...
1. Configuração global (`~/.config/aidev-cli/aidev-cli.json`)
2. Configuração do projeto (`aidev-cli.json`)
3. Diretório de plugins global (`~/.config/aidev-cli/plugins/`)
4. Diretório de plugins do projeto (`.aidev-cli/plugins/`)
...
O aidev-cli executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do aidev-cli para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o aidev-cli.
...
:::note
Se você estiver usando o aplicativo desktop aidev-cli, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o aidev-cli de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao aidev-cli:
...
O helper `tool` cria uma ferramenta personalizada que o aidev-cli pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o aidev-cli junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://aidev-cli.ai/docs/sdk) para detalhes.
