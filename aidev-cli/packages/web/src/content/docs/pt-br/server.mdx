---
title: Servidor
description: Interaja com o servidor aidev-cli via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `aidev-cli serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente aidev-cli pode usar.
...
```bash
aidev-cli serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `aidev-cli.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
aidev-cli serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `AIDEV_CLI_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `aidev-cli`, ou defina `AIDEV_CLI_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `aidev-cli serve` quanto ao `aidev-cli web`.

```bash
AIDEV_CLI_SERVER_PASSWORD=your-password aidev-cli serve
```
...
Quando você executa `aidev-cli`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor aidev-cli para interagir com o aidev-cli programaticamente.
:::

Essa arquitetura permite que o aidev-cli suporte múltiplos clientes e permite que você interaja com o aidev-cli programaticamente.

Você pode executar `aidev-cli serve` para iniciar um servidor autônomo. Se você tiver o TUI do aidev-cli em execução, `aidev-cli serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do aidev-cli [IDE](/docs/ide).
...
O servidor aidev-cli expõe as seguintes APIs.
