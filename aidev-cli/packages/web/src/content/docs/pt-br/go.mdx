...
O AidevCLI Go é uma assinatura de baixo custo de **US$ 10/mês** que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no AidevCLI. Você assina o AidevCLI Go e
...
O AidevCLI Go lhe dá acesso a esses modelos por **US$ 10/mês**.

---

## Como funciona

O AidevCLI Go funciona como qualquer outro provedor no AidevCLI.

1. Você entra no **<a href={console}>AidevCLI Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `AidevCLI Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o AidevCLI Go.
:::
...
O AidevCLI Go foi projetado para ser usado com o [AidevCLI](https://aidev-cli.ai) e outros agentes de programação populares que geram tipos de requisições semelhantes.
...
3\. inclui o header `x-aidev-cli-session` para que possamos otimizar o cache de prompts

## Limites de uso

O AidevCLI Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do AidevCLI
usa o formato `aidev-cli-go/<model-id>`. Por exemplo, para o Kimi K3, você usaria
`aidev-cli-go/kimi-k3` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o AidevCLI também.
