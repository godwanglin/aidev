...
Mit AidevCLI kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet AidevCLI das `aidev-cli`-Theme.
...
AidevCLI liefert mehrere eingebaute Themes mit.
...
- AidevCLI optisch ans Terminal anpassen wollen
...
AidevCLI unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/aidev-cli/themes/*.json` oder `$XDG_CONFIG_HOME/aidev-cli/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.aidev-cli/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.aidev-cli/themes/*.json`
