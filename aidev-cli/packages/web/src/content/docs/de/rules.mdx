---
title: Regeln
description: Lege eigene Anweisungen fuer aidev-cli fest.
---

Du kannst aidev-cli ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in aidev-cli den Befehl `/init` aus.
...
So kann aidev-cli sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht aidev-cli sie zu erweitern.
...
aidev-cli kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/aidev-cli/AGENTS.md` speichern.
Diese gelten in allen aidev-cli-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt AidevCLI dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/aidev-cli/AGENTS.md` existiert)
...
Beim Start sucht aidev-cli in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/aidev-cli/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `aidev-cli.json` oder in `~/.config/aidev-cli/aidev-cli.json` angeben.
...
aidev-cli parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit aidev-cli.json

Empfohlen ist das `instructions`-Feld in der `aidev-cli.json`:
...
Du kannst aidev-cli explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `aidev-cli.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
