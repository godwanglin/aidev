...
Agent Skills erlauben AidevCLI, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
AidevCLI sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft AidevCLI vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.aidev-cli/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/aidev-cli/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.aidev-cli/skills/git-release/SKILL.md` zum Beispiel so:
...
AidevCLI listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `aidev-cli.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `aidev-cli.json`):
