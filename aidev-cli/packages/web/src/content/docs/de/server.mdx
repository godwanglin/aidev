---
title: Server
description: Nutze den aidev-cli-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `aidev-cli serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein aidev-cli-Client nutzen kann.
...
Setze `AIDEV_CLI_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `aidev-cli`, kann aber mit `AIDEV_CLI_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `aidev-cli serve` und `aidev-cli web`.
...
Wenn du `aidev-cli` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den aidev-cli-Server, um aidev-cli programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `aidev-cli serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `aidev-cli serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die AidevCLI-[IDE](/docs/ide)-Plugins.
...
Der aidev-cli-Server stellt folgende APIs bereit.
