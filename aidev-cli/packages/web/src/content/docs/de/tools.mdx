...
AidevCLI bringt eingebaute Tools mit und laesst sich ueber [Custom Tools](/docs/custom-tools) oder [MCP-Server](/docs/mcp-servers) erweitern.
...
Hier sind alle in AidevCLI verfuegbaren eingebauten Tools.
...
:::note
Dieses Tool ist nur verfuegbar, wenn `AIDEV_CLI_EXPERIMENTAL_LSP_TOOL=true` (oder `AIDEV_CLI_EXPERIMENTAL=true`) gesetzt ist.
:::
...
:::note
Dieses Tool ist nur verfuegbar, wenn der AidevCLI-Provider genutzt wird oder die Umgebungsvariable `AIDEV_CLI_ENABLE_EXA` auf einen 'truthy' Wert (z. B. `true` oder `1`) gesetzt ist.

Zum Aktivieren beim Start von AidevCLI:
