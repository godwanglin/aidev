...
Wenn AidevCLI Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\aidev-cli\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `aidev-cli --log-level DEBUG`.

---

## Speicher

aidev-cli speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/aidev-cli/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\aidev-cli`
...
AidevCLI Desktop startet im Hintergrund einen lokalen AidevCLI-Server (`aidev-cli-cli`-Sidecar).
...
- Nur macOS: `AidevCLI`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/aidev-cli/aidev-cli.jsonc` (oder `~/.config/aidev-cli/aidev-cli.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/aidev-cli/aidev-cli.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\aidev-cli\aidev-cli.jsonc`
...
AidevCLI kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/aidev-cli/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\aidev-cli\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit AidevCLI ihn neu aufbauen kann.

1. Beende AidevCLI Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/aidev-cli`
- **Linux**: loesche `~/.cache/aidev-cli` (oder fuehre aus: `rm -rf ~/.cache/aidev-cli`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\aidev-cli`

3. Starte AidevCLI Desktop neu.

---

### Server-Verbindungsprobleme beheben

AidevCLI Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `aidev-cli.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `AIDEV_CLI_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `AIDEV_CLI_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt AidevCLI Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit AidevCLI-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

AidevCLI Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer AidevCLI in den OS-Einstellungen aktiviert sind, und
...
1. Beende AidevCLI Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von AidevCLI Desktop):

- `aidev-cli.settings.dat` (desktop default server URL)
- `aidev-cli.global.dat` und `aidev-cli.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit AidevCLI hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `aidev-cli models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\aidev-cli`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. aidev-cli installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\aidev-cli`

2. Starte aidev-cli neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt aidev-cli, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
aidev-cli erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
