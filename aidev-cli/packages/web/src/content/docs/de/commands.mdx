...
Erstellen Sie `.aidev-cli/commands/test.md`:

```md title=".aidev-cli/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die AidevCLI-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem AidevCLI [config](/docs/config):

```json title="aidev-cli.jsonc" {4-12}
{
...
- Global: `~/.config/aidev-cli/commands/`
- Pro Projekt: `.aidev-cli/commands/`

```markdown title="~/.config/aidev-cli/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".aidev-cli/commands/component.md"
---
...
```md title=".aidev-cli/commands/create-file.md"
---
...
```md title=".aidev-cli/commands/analyze-coverage.md"
---
...
```md title=".aidev-cli/commands/review-changes.md"
---
...
```md title=".aidev-cli/commands/review-component.md"
---
...
AidevCLI enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
