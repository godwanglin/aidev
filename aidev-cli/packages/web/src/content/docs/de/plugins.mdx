---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um AidevCLI zu erweitern.
---

Mit Plugins können Sie AidevCLI erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von AidevCLI zu ändern.
...
- `.aidev-cli/plugins/` – Plugins auf Projektebene
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/aidev-cli/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/aidev-cli/aidev-cli.json`)
2. Projektkonfiguration (`aidev-cli.json`)
3. Globales Plugin-Verzeichnis (`~/.config/aidev-cli/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.aidev-cli/plugins/`)
...
AidevCLI führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein AidevCLI-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie AidevCLI erweitern können.
...
:::note
Wenn Sie die AidevCLI-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass AidevCLI `.env`-Dateien liest:
...
Plugins können AidevCLI auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das AidevCLI aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für AidevCLI zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://aidev-cli.ai/docs/sdk).
