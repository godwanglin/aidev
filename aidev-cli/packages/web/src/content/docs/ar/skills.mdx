...
تتيح مهارات الوكيل لـ AidevCLI اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث AidevCLI في هذه المواقع:

- إعدادات المشروع: `.aidev-cli/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/aidev-cli/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج AidevCLI صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.aidev-cli/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/aidev-cli/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.aidev-cli/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض AidevCLI المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `aidev-cli.json`:
...
**للوكلاء المدمجين** (في `aidev-cli.json`):
