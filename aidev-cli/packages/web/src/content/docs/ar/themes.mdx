...
مع AidevCLI يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم AidevCLI سمتنا `aidev-cli`.
...
يأتي AidevCLI مع عدة سمات مدمجة.
...
- يريدون أن يطابق AidevCLI مظهر terminal لديهم
...
يدعم AidevCLI نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/aidev-cli/themes/*.json` أو `$XDG_CONFIG_HOME/aidev-cli/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.aidev-cli/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.aidev-cli/themes/*.json`
