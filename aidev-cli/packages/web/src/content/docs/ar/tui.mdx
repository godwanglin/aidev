---
title: TUI
description: استخدام واجهة المستخدم TUI في AidevCLI.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يوفّر AidevCLI واجهة terminal تفاعلية (TUI) للعمل على مشاريعك باستخدام LLM.

يؤدي تشغيل AidevCLI إلى بدء واجهة TUI للدليل الحالي.
...
عند استخدام واجهة AidevCLI في terminal، يمكنك كتابة `/` متبوعة باسم أمر لتنفيذ الإجراءات بسرعة. مثلا:
...
أضف موفّرا إلى AidevCLI. يتيح لك اختيار أحد الموفّرين المتاحين وإضافة مفاتيح API الخاصة بهم.
...
اخرج من AidevCLI. _الأسماء المستعارة_: `/quit`, `/q`
...
هذا منفصل عن `aidev-cli.json`، الذي يضبط سلوك الخادم ووقت التشغيل.
...
- `leader_timeout` - يتحكم في مدة انتظار AidevCLI بعد الضغط على leader key. القيمة الافتراضية `2000`.
...
- `sound_pack` - sound pack ID المراد استخدامه. القيمة الافتراضية `aidev-cli.default`.
