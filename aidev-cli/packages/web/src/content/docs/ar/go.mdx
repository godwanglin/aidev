...
AidevCLI Go هو اشتراك منخفض التكلفة بقيمة **$10/شهريًا** يمنحك وصولًا موثوقًا إلى نماذج البرمجة المفتوحة الشائعة.

يعمل Go مثل أي مزود آخر في AidevCLI. تشترك في AidevCLI Go وتحصل على مفتاح API الخاص بك. وهو **اختياري تمامًا**، ولا تحتاج إلى استخدامه لاستخدام AidevCLI.
...
:::tip
اختبرنا مجموعة مختارة من النماذج والمزودين الذين يعملون جيدًا مع AidevCLI.
:::
...
يمنحك AidevCLI Go الوصول إلى هذه النماذج مقابل **$10/شهريًا**.

---

## كيف يعمل

يعمل AidevCLI Go مثل أي مزود آخر في AidevCLI.

1. تسجّل الدخول إلى **<a href={console}>AidevCLI Zen</a>**، وتشترك في Go، ثم تنسخ مفتاح API الخاص بك.
2. تشغّل الأمر `/connect` في TUI، وتختار `AidevCLI Go`، ثم تلصق مفتاح API الخاص بك.
3. شغّل `/models` في TUI لرؤية قائمة النماذج المتاحة عبر Go.

:::note
يمكن لعضو واحد فقط في كل workspace الاشتراك في AidevCLI Go.
:::
...
صُمّم AidevCLI Go للاستخدام مع [AidevCLI](https://aidev-cli.ai) وغيره من وكلاء البرمجة الشائعين الذين ينشئون أنواعًا مماثلة من الطلبات.

تتم مراقبة حركة المرور لرصد الاستخدام المسيء الذي يؤدي إلى تدهور تجربة المستخدمين الآخرين.

لتجنب وضع علامة على حسابك، تأكد من أن الأداة التي تستخدمها

1\. لا تنشئ حركة مرور مسيئة  
2\. تعرّف عن نفسها بشكل صحيح (من دون معرّفات وكيل مستخدم عامة)  
3\. تتضمن ترويسة `x-aidev-cli-session` حتى نتمكن من تحسين التخزين المؤقت للمطالبات

## حدود الاستخدام

يتضمن AidevCLI Go الحدود التالية:
...
يستخدم [model id](/docs/config/#models) في إعدادات AidevCLI لديك التنسيق `aidev-cli-go/<model-id>`. على سبيل المثال، بالنسبة إلى Kimi K3، ستستخدم `aidev-cli-go/kimi-k3` في إعداداتك.
...
4. **عدم فرض قيود ارتباط** من خلال السماح لك أيضًا باستخدام أي مزود آخر مع AidevCLI.
