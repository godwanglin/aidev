---
title: المزوّدون
description: استخدام أي موفّر LLM في AidevCLI.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم AidevCLI ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات AidevCLI.
...
في `~/.local/share/aidev-cli/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات AidevCLI.
...
AidevCLI Zen هي قائمة نماذج يوفّرها فريق AidevCLI وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع AidevCLI. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع AidevCLI Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر aidev-cli، ثم انتقل إلى [aidev-cli.ai/auth](https://aidev-cli.ai/auth).
...
يعمل مثل أي مزوّد آخر في AidevCLI واستخدامه اختياري بالكامل.

---

## AidevCLI Go

AidevCLI Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق AidevCLI، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع AidevCLI.

1. شغّل الأمر `/connect` في TUI، واختر `AidevCLI Go`، ثم انتقل إلى [aidev-cli.ai/auth](https://aidev-cli.ai/zen).
...
لاستخدام Amazon Bedrock مع AidevCLI:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل aidev-cli:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `aidev-cli.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل AidevCLI غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين aidev-cli لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل aidev-cli بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات aidev-cli.
...
تخزين الرمز في مساحة تخزين مصادقة aidev-cli.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم AidevCLI نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر AidevCLI
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`aidev-cli.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [aidev-cli-gitlab-auth](https://www.npmjs.com/package/aidev-cli-gitlab-auth).

##### التهيئة

خصّص عبر `aidev-cli.json`:
...
لاستخدام اشتراك GitHub Copilot مع aidev-cli:
...
لاستخدام Google Vertex AI مع AidevCLI:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر aidev-cli، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [aidev-cli-helicone-session](https://github.com/H2Shami/aidev-cli-helicone-session) لتسجيل كل محادثة AidevCLI تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة AidevCLI مدرجة كجلسة مستقلة.
...
يمكنك ضبط aidev-cli لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط aidev-cli لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط aidev-cli لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع AidevCLI. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/aidev-cli) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع AidevCLI:
...
3. انسخ مفتاح API لاستخدامه في AidevCLI.
...
6. **مهم**: قبل استخدام نماذج السحابة في AidevCLI، يجب سحب معلومات النموذج محليا:
...
AidevCLI Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق AidevCLI. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>AidevCLI Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **AidevCLI Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ AidevCLI.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات aidev-cli.
...
5. يمكنك أيضا تخصيصها عبر إعدادات aidev-cli. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات aidev-cli. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع aidev-cli. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `aidev-cli.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ AidevCLI بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `aidev-cli auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات aidev-cli وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات aidev-cli.
