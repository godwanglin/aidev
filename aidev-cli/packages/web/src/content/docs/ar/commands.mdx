...
أنشئ الملف `.aidev-cli/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات AidevCLI أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ AidevCLI:
...
- على مستوى النظام: `~/.config/aidev-cli/commands/`
- لكل مشروع: `.aidev-cli/commands/`
...
يتضمن aidev-cli عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
