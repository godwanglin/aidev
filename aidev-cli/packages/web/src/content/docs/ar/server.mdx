---
title: الخادم
description: تفاعل مع خادم aidev-cli عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `aidev-cli serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل aidev-cli استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `aidev-cli.local` |
...
عيّن `AIDEV_CLI_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `aidev-cli`، أو عيّن `AIDEV_CLI_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `aidev-cli serve` و `aidev-cli web`.
...
عند تشغيل `aidev-cli` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم aidev-cli للتفاعل مع aidev-cli برمجيا.
:::

تتيح هذه البنية لـ aidev-cli دعم عدة عملاء وتمكّنك من التفاعل مع aidev-cli برمجيا.

يمكنك تشغيل `aidev-cli serve` لبدء خادم مستقل. إذا كانت واجهة aidev-cli في terminal (TUI)
قيد التشغيل، فسيبدأ `aidev-cli serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات AidevCLI لـ [IDE](/docs/ide).
...
يعرض خادم aidev-cli واجهات API التالية.
