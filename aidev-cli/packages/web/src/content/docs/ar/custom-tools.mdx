...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في aidev-cli.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في aidev-cli مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.aidev-cli/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/aidev-cli/tools/`.
