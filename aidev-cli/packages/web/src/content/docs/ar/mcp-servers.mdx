...
يمكنك إضافة أدوات خارجية إلى AidevCLI باستخدام _Model Context Protocol_، أو MCP. يدعم AidevCLI خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات AidevCLI](https://aidev-cli.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/aidev-cli` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى AidevCLI تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم AidevCLI بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك AidevCLI المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `aidev-cli mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن AidevCLI الرموز بشكل آمن في `~/.local/share/aidev-cli/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل AidevCLI إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات AidevCLI مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط AidevCLI بحساب Sentry الخاص بك.
