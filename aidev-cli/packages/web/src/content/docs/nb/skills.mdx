...
Agentferdigheter lar AidevCLI oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
aidev-cli søker etter disse stedene:

- Prosjektkonfigurasjon: `.aidev-cli/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/aidev-cli/skills/<name>/SKILL.md`
...
For prosjektlokale stier går aidev-cli opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.aidev-cli/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/aidev-cli/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.aidev-cli/skills/git-release/SKILL.md` slik:
...
aidev-cli viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `aidev-cli.json`:
...
**For innebygde agenter** (i `aidev-cli.json`):
