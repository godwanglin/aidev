...
Det er to typer agenter i AidevCLI; primære agenter og underagenter.
...
AidevCLI kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
AidevCLI kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

AidevCLI kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `aidev-cli.json`:
...
Hvis ingen temperatur er spesifisert, bruker AidevCLI modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale AidevCLI-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i AidevCLI-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [AidevCLI Zen](/docs/zen), vil du bruke `aidev-cli/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `aidev-cli models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/aidev-cli).
:::
