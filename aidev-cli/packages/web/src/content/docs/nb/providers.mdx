---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i AidevCLI.
---

import config from "../../../../config.mjs"
export const console = config.console

AidevCLI bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i AidevCLI-konfigurasjonen.
...
i `~/.local/share/aidev-cli/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i AidevCLI
konfig.
...
AidevCLI Zen er en liste over modeller levert av AidevCLI-teamet som har vært
testet og verifisert for å fungere godt med AidevCLI. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med AidevCLI Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `AidevCLI Zen` og gå til [aidev-cli.ai/auth](https://aidev-cli.ai/zen).
...
Det fungerer som alle andre leverandører i AidevCLI og er helt valgfritt å bruke.

---

## AidevCLI Go

AidevCLI Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av AidevCLI-teamet som har vært
testet og verifisert for å fungere godt med AidevCLI.

1. Kjør kommandoen `/connect` i TUI, velg `AidevCLI Go`, og gå til [aidev-cli.ai/auth](https://aidev-cli.ai/zen).
...
Slik bruker du Amazon Bedrock med AidevCLI:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører AidevCLI:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `aidev-cli.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i aidev-cli støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere aidev-cli til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at AidevCLI skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom AidevCLI-konfigurasjonen.
...
for å lagre token i AidevCLI auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
AidevCLI bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse AidevCLI
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`aidev-cli.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [aidev-cli-gitlab-auth](https://www.npmjs.com/package/aidev-cli-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `aidev-cli.json`:
...
Slik bruker du GitHub Copilot-abonnementet med AidevCLI:
...
Slik bruker du Google Vertex AI med AidevCLI:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører AidevCLI.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom AidevCLI, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [aidev-cli-helicone-session](https://github.com/H2Shami/aidev-cli-helicone-session) plugin for automatisk å logge hver AidevCLI-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver AidevCLI-samtale oppført som en separat økt.
...
Du kan konfigurere AidevCLI for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere AidevCLI for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere AidevCLI for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for AidevCLI. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/aidev-cli) for detaljer.
:::
...
Slik bruker du Ollama Cloud med AidevCLI:
...
3. Kopier API-nøkkelen for bruk i AidevCLI.
...
6. **Viktig**: Før du bruker skymodeller i AidevCLI, må du hente modellinformasjonen lokalt:
...
AidevCLI Zen er en liste over testede og verifiserte modeller levert av AidevCLI-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>AidevCLI Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **AidevCLI Zen**.

   ```txt
   /connect
   ```

3. Skriv inn AidevCLI API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom AidevCLI-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom AidevCLI-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med AidevCLI:
...
Du kan også tilpasse modeller gjennom AidevCLI-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med AidevCLI. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `aidev-cli.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar AidevCLI forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `aidev-cli auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk AidevCLI-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i AidevCLI-konfigurasjonen.
