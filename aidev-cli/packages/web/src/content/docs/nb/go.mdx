...
AidevCLI Go er et lavkostnadsabonnement til **$10/måned** som gir deg pålitelig tilgang til populære åpne kodemodeller.

Go fungerer som enhver annen leverandør i AidevCLI. Du abonnerer på AidevCLI Go og
...
AidevCLI Go gir deg tilgang til disse modellene for **$10/måned**.

---

## Hvordan det fungerer

AidevCLI Go fungerer som enhver annen leverandør i AidevCLI.

1. Du logger inn på **<a href={console}>AidevCLI Zen</a>**, abonnerer på Go, og
   kopierer din API-nøkkel.
2. Du kjører kommandoen `/connect` i TUI-en, velger `AidevCLI Go`, og limer inn
   din API-nøkkel.
3. Kjør `/models` i TUI-en for å se listen over modeller som er tilgjengelige gjennom Go.

:::note
Kun ett medlem per arbeidsområde kan abonnere på AidevCLI Go.
:::
...
AidevCLI Go er utviklet for bruk med [AidevCLI](https://aidev-cli.ai) og andre populære kodeagenter som genererer lignende typer forespørsler.

Trafikken overvåkes for misbruk som forringer opplevelsen for andre brukere.

For å sikre at kontoen din ikke blir flagget, må du sørge for at verktøyet du bruker,

1\. ikke genererer misbrukstrafikk  
2\. identifiserer seg korrekt (ingen generiske User-Agent-identifikatorer)  
3\. inkluderer `x-aidev-cli-session`-headeren, slik at vi kan optimalisere promptbufring

## Bruksgrenser

AidevCLI Go inkluderer følgende grenser:
...
[Modell-ID-en](/docs/config/#models) i din AidevCLI-konfigurasjon
bruker formatet `aidev-cli-go/<model-id>`. For eksempel, for Kimi K3, vil du
bruke `aidev-cli-go/kimi-k3` i konfigurasjonen din.
...
Vi opprettet AidevCLI Go for å:
...
4. Ha **ingen innelåsing** ved å la deg bruke en hvilken som helst annen leverandør med AidevCLI også.
