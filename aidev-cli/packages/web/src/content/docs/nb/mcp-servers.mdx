...
Du kan legge til eksterne verktøy i AidevCLI ved å bruke _Model Context Protocol_ eller MCP. AidevCLI støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [AidevCLI Config](https://aidev-cli.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="aidev-cli.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/aidev-cli`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="aidev-cli.jsonc" {15}
{
  "$schema": "https://aidev-cli.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
AidevCLI håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil AidevCLI:
...
Hvis serveren krever autentisering, vil AidevCLI be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `aidev-cli mcp auth <server-name>`.
...
```json title="aidev-cli.json" {7-11}
{
  "$schema": "https://aidev-cli.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
aidev-cli mcp auth my-oauth-server
```
...
```bash
aidev-cli mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil AidevCLI lagre tokens sikkert i `~/.local/share/aidev-cli/mcp-auth.json`.
...
aidev-cli mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i AidevCLI, sammen med innebygde verktøy. Så du kan administrere dem gjennom AidevCLI-konfigurasjonen som alle andre verktøy.
...
```json title="aidev-cli.json" {11, 14-18}
{
  "$schema": "https://aidev-cli.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
aidev-cli mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble AidevCLI til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver AidevCLI-samtale oppført som en separat økt.
