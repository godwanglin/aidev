---
title: Tjener
description: Samhandle med AidevCLI-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `aidev-cli serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en AidevCLI-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `aidev-cli.local` |
...
Sett `AIDEV_CLI_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `aidev-cli` som standard, eller sett `AIDEV_CLI_SERVER_USERNAME` for å overstyre det. Dette gjelder både `aidev-cli serve` og `aidev-cli web`.
...
Når du kjører `aidev-cli` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk AidevCLI-serveren til å samhandle med AidevCLI programmatisk.
:::

Denne arkitekturen lar AidevCLI støtte flere klienter og lar deg samhandle med AidevCLI programmatisk.

Du kan kjøre `aidev-cli serve` for å starte en frittstående server. Hvis du har
AidevCLI TUI kjører, vil `aidev-cli serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av AidevCLI [IDE](/docs/ide) plugins.
...
aidev-cli-serveren viser følgende APIer.
