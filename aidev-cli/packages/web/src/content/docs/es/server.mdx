---
title: Servidor
description: Interactuar con el servidor aidev-cli a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `aidev-cli serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente aidev-cli puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `aidev-cli.local` |
...
Configure `AIDEV_CLI_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `aidev-cli`, o configure `AIDEV_CLI_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `aidev-cli serve` como a `aidev-cli web`.
...
Cuando ejecuta `aidev-cli`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor aidev-cli para interactuar con aidev-cli mediante programación.
:::

Esta arquitectura permite que aidev-cli admita múltiples clientes y le permite interactuar con aidev-cli mediante programación.

Puede ejecutar `aidev-cli serve` para iniciar un servidor independiente. Si tienes el
aidev-cli TUI ejecutándose, `aidev-cli serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos AidevCLI [IDE](/docs/ide).
...
El servidor aidev-cli expone las siguientes API.
