---
title: Complementos
description: Escriba sus propios complementos para extender AidevCLI.
---

Los complementos le permiten extender AidevCLI al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de AidevCLI.
...
- `.aidev-cli/plugins/` - Complementos a nivel de proyecto
- `~/.config/aidev-cli/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/aidev-cli/node_modules/`.
...
1. Configuración global (`~/.config/aidev-cli/aidev-cli.json`)
2. Configuración del proyecto (`aidev-cli.json`)
3. Directorio global de complementos (`~/.config/aidev-cli/plugins/`)
4. Directorio de complementos del proyecto (`.aidev-cli/plugins/`)
...
AidevCLI ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK aidev-cli para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar aidev-cli.
...
:::note
Si está utilizando la aplicación de escritorio AidevCLI, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que aidev-cli lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a aidev-cli:
...
El ayudante `tool` crea una herramienta personalizada a la que aidev-cli puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para aidev-cli junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://aidev-cli.ai/docs/sdk) para obtener más detalles.
