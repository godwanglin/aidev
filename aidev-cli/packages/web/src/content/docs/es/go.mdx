...
AidevCLI Go es una suscripción de bajo costo de **$10/mes** que te brinda acceso confiable a modelos abiertos de programación populares.

Go funciona como cualquier otro proveedor en AidevCLI. Te suscribes a AidevCLI Go y
...
AidevCLI Go te da acceso a estos modelos por **$10/mes**.

---

## Cómo funciona

AidevCLI Go funciona como cualquier otro proveedor en AidevCLI.

1. Inicias sesión en **<a href={console}>AidevCLI Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `AidevCLI Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a AidevCLI Go.
:::
...
AidevCLI Go está diseñado para usarse con [AidevCLI](https://aidev-cli.ai) y otros agentes de programación populares que generan tipos de peticiones similares.
...
3\. incluya el encabezado `x-aidev-cli-session` para que podamos optimizar el almacenamiento en caché de prompts

## Límites de uso

AidevCLI Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de AidevCLI
usa el formato `aidev-cli-go/<model-id>`. Por ejemplo, para Kimi K3, usarías
`aidev-cli-go/kimi-k3` en tu configuración.
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con AidevCLI.
