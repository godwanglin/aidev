...
Con AidevCLI puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, AidevCLI usa nuestro propio tema `aidev-cli`.
...
AidevCLI viene con varios temas integrados.
...
- Quiere que AidevCLI coincida con la apariencia de su terminal
...
AidevCLI admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/aidev-cli/themes/*.json` o `$XDG_CONFIG_HOME/aidev-cli/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.aidev-cli/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.aidev-cli/themes/*.json`
