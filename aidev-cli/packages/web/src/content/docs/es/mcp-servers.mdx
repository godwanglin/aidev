...
Puede agregar herramientas externas a AidevCLI utilizando el _Model Context Protocol_ o MCP. AidevCLI admite servidores locales y remotos.
...
Puede definir servidores MCP en su [AidevCLI Config](https://aidev-cli.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/aidev-cli`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
AidevCLI maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, AidevCLI:
...
Si el servidor requiere autenticación, AidevCLI le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `aidev-cli mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, AidevCLI almacenará los tokens de forma segura en `~/.local/share/aidev-cli/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en AidevCLI, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración AidevCLI como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar AidevCLI a su cuenta Sentry.
