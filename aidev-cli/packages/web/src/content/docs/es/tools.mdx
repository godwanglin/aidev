...
Las herramientas permiten que LLM realice acciones en su código base. AidevCLI viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en AidevCLI.
...
:::note
Esta herramienta solo está disponible cuando `AIDEV_CLI_EXPERIMENTAL_LSP_TOOL=true` (o `AIDEV_CLI_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor AidevCLI o cuando la variable de entorno `AIDEV_CLI_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar AidevCLI:

```bash
AIDEV_CLI_ENABLE_EXA=1 aidev-cli
```
