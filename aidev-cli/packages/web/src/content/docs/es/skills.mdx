...
Las habilidades del agente permiten a AidevCLI descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
AidevCLI busca estas ubicaciones:

- Configuración del proyecto: `.aidev-cli/skills/<name>/SKILL.md`
- Configuración global: `~/.config/aidev-cli/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, AidevCLI sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.aidev-cli/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/aidev-cli/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.aidev-cli/skills/git-release/SKILL.md` así:
...
AidevCLI enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `aidev-cli.json`:
...
**Para agentes integrados** (en `aidev-cli.json`):
