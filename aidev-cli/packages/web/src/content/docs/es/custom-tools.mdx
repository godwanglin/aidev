...
description: Cree herramientas que LLM pueda llamar en aidev-cli.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de aidev-cli como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.aidev-cli/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/aidev-cli/tools/`.
...
```ts title=".aidev-cli/tools/database.ts" {1}
...
```ts title=".aidev-cli/tools/math.ts"
import { tool } from "@aidev-cli/plugin"
...
```ts title=".aidev-cli/tools/bash.ts"
import { tool } from "@aidev-cli/plugin"
...
```ts title=".aidev-cli/tools/project.ts" {8}
import { tool } from "@aidev-cli/plugin"
...
```python title=".aidev-cli/tools/add.py"
import sys
...
```ts title=".aidev-cli/tools/python-add.ts" {10}
import { tool } from "@aidev-cli/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".aidev-cli/tools/add.py")
