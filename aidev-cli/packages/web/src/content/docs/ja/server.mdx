---
title: サーバー
description: HTTP 経由で AidevCLI サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`aidev-cli serve` コマンドは、AidevCLI クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `aidev-cli.local` |
...
HTTP 基本認証でサーバーを保護するには、`AIDEV_CLI_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `aidev-cli` になるか、`AIDEV_CLI_SERVER_USERNAME` を設定してオーバーライドします。これは、`aidev-cli serve` と `aidev-cli web` の両方に当てはまります。
...
`aidev-cli` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
AidevCLI サーバーを使用して、プログラムで AidevCLI と対話します。
:::

`aidev-cli serve` を実行してスタンドアロンサーバーを起動できます。すでに AidevCLI TUI を実行している場合でも、`aidev-cli serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、AidevCLI [IDE](/docs/ide) プラグインによって使用されます。
