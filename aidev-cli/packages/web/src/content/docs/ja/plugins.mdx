---
title: プラグイン
description: AidevCLI を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで AidevCLI を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、AidevCLI のデフォルトの動作を変更したりできます。
...
- `.aidev-cli/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/aidev-cli/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/aidev-cli/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/aidev-cli/aidev-cli.json`)
2. プロジェクト設定 (`aidev-cli.json`)
3. グローバルプラグインディレクトリ (`~/.config/aidev-cli/plugins/`)
4. プロジェクトプラグインディレクトリ (`.aidev-cli/plugins/`)
...
AidevCLI は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための AidevCLI SDK クライアント。
...
ここでは、AidevCLI を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://aidev-cli.ai/docs/sdk) を参照してください。
...
AidevCLI が `.env` ファイルを読み取らないようにします。
...
プラグインは AidevCLI にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、AidevCLI が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して AidevCLI で使用できます。
