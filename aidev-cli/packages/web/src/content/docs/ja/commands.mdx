...
`.aidev-cli/commands/test.md` を作成します。
...
カスタムコマンドは、AidevCLI 設定を通じて、または `commands/` ディレクトリに Markdown ファイルを作成することによって追加できます。

---

### JSON

AidevCLI [設定](/docs/config)で `command` オプションを使用します:
...
- グローバル: `~/.config/aidev-cli/commands/`
- プロジェクトごと: `.aidev-cli/commands/`
...
AidevCLI には、`/init`、`/undo`、`/redo`、`/share`、`/help` などのいくつかの組み込みコマンドが含まれています。 [詳細はこちら](/docs/tui#commands)。
