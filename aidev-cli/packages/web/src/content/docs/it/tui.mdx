---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di AidevCLI.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

AidevCLI fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire AidevCLI avvia la TUI per la directory corrente.
...
Quando usi la TUI di AidevCLI, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a AidevCLI. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da AidevCLI. _Alias_: `/quit`, `/q`
...
Questo è separato da `aidev-cli.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo AidevCLI attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `aidev-cli.default`.
