---
title: Server
description: Interagisci con il server aidev-cli via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `aidev-cli serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client aidev-cli.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `aidev-cli.local` |
...
Imposta `AIDEV_CLI_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `aidev-cli`, oppure imposta `AIDEV_CLI_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `aidev-cli serve` sia per `aidev-cli web`.
...
Quando esegui `aidev-cli` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server aidev-cli per interagire con aidev-cli in modo programmatico.
:::

Questa architettura permette a aidev-cli di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `aidev-cli serve` per avviare un server standalone. Se la TUI di aidev-cli e gia in esecuzione, `aidev-cli serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di AidevCLI.
...
Il server aidev-cli espone le seguenti API.
