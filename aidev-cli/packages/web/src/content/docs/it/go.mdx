...
AidevCLI Go è un abbonamento a basso costo da **10 $/mese** che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in AidevCLI. Ti abboni a AidevCLI Go e
...
AidevCLI Go ti dà accesso a questi modelli a **10 $/mese**.

---

## Come funziona

AidevCLI Go funziona come qualsiasi altro provider in AidevCLI.

1. Accedi a **<a href={console}>AidevCLI Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `AidevCLI Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a AidevCLI Go.
:::
...
AidevCLI Go è progettato per essere utilizzato con [AidevCLI](https://aidev-cli.ai) e altri agenti di programmazione popolari che generano tipi di richieste simili.

Il traffico viene monitorato per rilevare traffico abusivo che compromette l'esperienza degli altri utenti.

Per evitare che il tuo account venga segnalato, assicurati che lo strumento che utilizzi

1\. non generi traffico abusivo  
2\. si identifichi correttamente (senza user agent generici)  
3\. includa l'header `x-aidev-cli-session` in modo da consentirci di ottimizzare il caching dei prompt

## Limiti di utilizzo

AidevCLI Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua AidevCLI config
utilizza il formato `aidev-cli-go/<model-id>`. Ad esempio, per Kimi K3, useresti
`aidev-cli-go/kimi-k3` nella tua configurazione.
...
Abbiamo creato AidevCLI Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con AidevCLI.
