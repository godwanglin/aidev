---
title: Plugin
description: Scrivi plugin per estendere AidevCLI.
---

I plugin ti permettono di estendere AidevCLI agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di AidevCLI.
...
- `.aidev-cli/plugins/` - plugin a livello progetto
- `~/.config/aidev-cli/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/aidev-cli/node_modules/`.
...
1. Config globale (`~/.config/aidev-cli/aidev-cli.json`)
2. Config di progetto (`aidev-cli.json`)
3. Directory plugin globale (`~/.config/aidev-cli/plugins/`)
4. Directory plugin di progetto (`.aidev-cli/plugins/`)
...
AidevCLI esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di aidev-cli per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere aidev-cli.
...
:::note
Se usi l'app desktop di AidevCLI, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a aidev-cli di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a aidev-cli:
...
L'helper `tool` crea uno strumento personalizzato che aidev-cli può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in aidev-cli insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://aidev-cli.ai/docs/sdk) per i dettagli.
