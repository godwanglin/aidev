...
Le skill degli agenti permettono a AidevCLI di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
AidevCLI cerca in queste posizioni:

- Config di progetto: `.aidev-cli/skills/<name>/SKILL.md`
- Config globale: `~/.config/aidev-cli/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, AidevCLI risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.aidev-cli/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/aidev-cli/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.aidev-cli/skills/git-release/SKILL.md` cosi':
...
AidevCLI elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `aidev-cli.json`:
...
**Per agenti integrati** (in `aidev-cli.json`):
