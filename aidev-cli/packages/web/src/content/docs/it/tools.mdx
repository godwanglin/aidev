...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. AidevCLI include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in AidevCLI.
...
:::note
Questo strumento e' disponibile solo quando `AIDEV_CLI_EXPERIMENTAL_LSP_TOOL=true` (oppure `AIDEV_CLI_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider AidevCLI o quando la variabile d'ambiente `AIDEV_CLI_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di AidevCLI:
