...
Puoi aggiungere strumenti esterni a AidevCLI usando il _Model Context Protocol_, o MCP. AidevCLI supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [AidevCLI Config](https://aidev-cli.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/aidev-cli`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
AidevCLI gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, AidevCLI:
...
Se il server richiede autenticazione, AidevCLI ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `aidev-cli mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, AidevCLI memorizzera' i token in modo sicuro in `~/.local/share/aidev-cli/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in AidevCLI insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione AidevCLI come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare AidevCLI al tuo account Sentry.
