...
Crea `.aidev-cli/commands/test.md`:
...
Puoi aggiungere comandi personalizzati tramite la config di AidevCLI o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di AidevCLI:
...
- Globale: `~/.config/aidev-cli/commands/`
- Per progetto: `.aidev-cli/commands/`
...
aidev-cli include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).
