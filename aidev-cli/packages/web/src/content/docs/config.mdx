---
title: Config
description: Using the AidevCLI JSON config.
---

You can configure AidevCLI using a JSON config file.

---

## Format

AidevCLI supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="aidev-cli.jsonc"
{
...
1. **Remote config** (from `.well-known/aidev-cli`) - organizational defaults
2. **Global config** (`~/.config/aidev-cli/aidev-cli.json`) - user preferences
3. **Custom config** (`AIDEV_CLI_CONFIG` env var) - custom overrides
4. **Project config** (`aidev-cli.json` in project) - project-specific settings
5. **`.aidev-cli` directories** - agents, commands, plugins
6. **Inline config** (`AIDEV_CLI_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/aidev-cli/` on macOS) - admin-controlled
...
:::note
The `.aidev-cli` and `~/.config/aidev-cli` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/aidev-cli` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/aidev-cli"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global AidevCLI config in `~/.config/aidev-cli/aidev-cli.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/aidev-cli/tui.json`.
...
Add `aidev-cli.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When AidevCLI starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `AIDEV_CLI_CONFIG` environment variable.

```bash
export AIDEV_CLI_CONFIG=/path/to/my/custom-config.json
aidev-cli run "Hello world"
```
...
Specify a custom config directory using the `AIDEV_CLI_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.aidev-cli` directory, and should
follow the same structure.

```bash
export AIDEV_CLI_CONFIG_DIR=/path/to/my/config-directory
aidev-cli run "Hello world"
```

The custom directory is loaded after the global config and `.aidev-cli` directories, so it **can override** their settings.
...
Drop an `aidev-cli.json` or `aidev-cli.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/aidev-cli/` |
| Linux    | `/etc/aidev-cli/`                         |
| Windows  | `%ProgramData%\aidev-cli`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, AidevCLI reads managed preferences from the `ai.aidev-cli.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

AidevCLI checks these paths:

1. `/Library/Managed Preferences/<user>/ai.aidev-cli.managed.plist`
2. `/Library/Managed Preferences/ai.aidev-cli.managed.plist`

The plist keys map directly to `aidev-cli.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.aidev-cli.managed` PayloadType. The AidevCLI config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.aidev-cli.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.aidev-cli.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.aidev-cli</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`aidev-cli.ai/config.json`**](https://aidev-cli.ai/config.json).

TUI config uses [**`aidev-cli.ai/tui.json`**](https://aidev-cli.ai/tui.json).
...
Use `AIDEV_CLI_TUI_CONFIG` to point to a custom TUI config file.
...
Legacy `theme`, `keybinds`, and `tui` keys in `aidev-cli.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `aidev-cli serve` and `aidev-cli web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your AidevCLI server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `aidev-cli.local`. Useful for running multiple instances on the same network.
...
If not specified, AidevCLI will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your AidevCLI config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, AidevCLI tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny AidevCLI actions on configured resources. Currently, policies can control which providers AidevCLI may use.
...
AidevCLI normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, AidevCLI omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/aidev-cli/agents/` or `.aidev-cli/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, AidevCLI will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`aidev-cli run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/aidev-cli/commands/` or `.aidev-cli/commands/`. [Learn more here](/docs/commands).
...
AidevCLI uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
AidevCLI will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, aidev-cli **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend AidevCLI with custom tools, hooks, and integrations.

Place plugin files in `.aidev-cli/plugins/` or `~/.config/aidev-cli/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="aidev-cli.json"
{
  "$schema": "https://aidev-cli.ai/config.json",
  "plugin": ["aidev-cli-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict AidevCLI to only use specific providers rather than disabling them one by one.
