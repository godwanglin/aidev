---
title: Sağlayıcılar
description: aidev-cli'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

aidev-cli, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı aidev-cli yapılandırmanızda yapılandırın.
...
`~/.local/share/aidev-cli/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları aidev-cli'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
AidevCLI Zen, aidev-cli ekibi tarafından sağlanan modellerin bir listesidir.
aidev-cli ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz AidevCLI Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, aidev-cli'u seçin ve [aidev-cli.ai/auth](https://aidev-cli.ai/auth)'ye gidin.
...
aidev-cli'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## AidevCLI Go

AidevCLI Go, aidev-cli ile iyi çalıştığı test edilmiş ve doğrulanmış, aidev-cli ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `AidevCLI Go`'yu seçin ve [aidev-cli.ai/auth](https://aidev-cli.ai/zen) adresine gidin.
...
Amazon Bedrock'u aidev-cli ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   aidev-cli'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `aidev-cli.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi aidev-cli'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
aidev-cli'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   aidev-cli'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
aidev-cli, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. aidev-cli'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`aidev-cli.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [aidev-cli-gitlab-auth](https://www.npmjs.com/package/aidev-cli-gitlab-auth) ana sayfasında.

##### Yapılandırma

`aidev-cli.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi aidev-cli'la kullanmak için:
...
Google Vertex AI'yi aidev-cli ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları aidev-cli çalıştırılırken ayarlayın.
...
Helicone'un aidev-cli aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her aidev-cli görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [aidev-cli-helicone-session](https://github.com/H2Shami/aidev-cli-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her aidev-cli görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
aidev-cli'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla aidev-cli'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla aidev-cli'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini aidev-cli için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/aidev-cli)'a bakın.
:::
...
Ollama Cloud'u aidev-cli ile kullanmak için:
...
3. aidev-cli'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: aidev-cli'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
AidevCLI Zen, aidev-cli ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>AidevCLI Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **AidevCLI Zen**'i arayın.

   ```txt
   /connect
   ```

3. aidev-cli API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   aidev-cli yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca aidev-cli yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri aidev-cli yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı aidev-cli ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `aidev-cli.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları aidev-cli'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `aidev-cli auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için aidev-cli yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, aidev-cli yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
