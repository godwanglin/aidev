---
title: Özel Araçlar
description: LLM'un aidev-cli'da çağırabileceği araçları birleştirebilir.
---

Özel araçlar, LLM'un konuşmalar sırasında arayabileceği, oluşturduğunuz işlevlerdir. aidev-cli'un [built-in tools](/docs/tools) ile birlikte `read`, `write` ve `bash` gibi çalışırlar.
...
- Yerel olarak bunları projenizin `.aidev-cli/tools/` dizinine yerleştirerek.
- Veya küresel olarak bunları `~/.config/aidev-cli/tools/` içine yerleştirerek.
