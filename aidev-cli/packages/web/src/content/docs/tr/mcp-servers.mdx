...
_Model Bağlam Protokolü_ veya MCP kullanarak aidev-cli'a harici araçlar ekleyebilirsiniz. aidev-cli hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [aidev-cli Config](https://aidev-cli.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/aidev-cli` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
aidev-cli, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde aidev-cli şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa aidev-cli, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `aidev-cli mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından aidev-cli, belirteçleri `~/.local/share/aidev-cli/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra aidev-cli'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi aidev-cli yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve aidev-cli'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
