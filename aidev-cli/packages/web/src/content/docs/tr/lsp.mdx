...
description: AidevCLI, LSP sunucularınızla bütünleşir.
---

AidevCLI, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

AidevCLI, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`AIDEV_CLI_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve aidev-cli bir dosyayı açtığında:
...
LSP sunucularını, aidev-cli yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
