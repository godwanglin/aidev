...
aidev-cli ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak aidev-cli kendi `aidev-cli` temasını kullanır.
...
aidev-cli birden fazla yerleşik temayla gelir.
...
- aidev-cli'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
aidev-cli, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/aidev-cli/themes/*.json` veya `$XDG_CONFIG_HOME/aidev-cli/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.aidev-cli/themes/*.json`
4. **Geçerli çalışma dizini** - `./.aidev-cli/themes/*.json`
