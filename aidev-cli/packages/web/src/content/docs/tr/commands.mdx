...
`.aidev-cli/commands/test.md` oluştur:
...
aidev-cli işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

aidev-cli [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/aidev-cli/commands/`
- Proje başına: `.aidev-cli/commands/`
...
aidev-cli, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
