...
AidevCLI Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli **aylık 10$ aboneliğidir**.

Go, AidevCLI'daki diğer sağlayıcılar gibi çalışır. AidevCLI Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve AidevCLI'u kullanmak için bunu kullanmanıza gerek yoktur.
...
AidevCLI Go, bu modellere **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

AidevCLI Go, AidevCLI'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>AidevCLI Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `AidevCLI Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye AidevCLI Go'ya abone olabilir.
:::
...
AidevCLI Go, [AidevCLI](https://aidev-cli.ai) ve benzer türde istekler üreten diğer popüler kodlama aracılarıyla kullanılmak üzere tasarlanmıştır.
...
1\. kötüye kullanım amaçlı trafik oluşturmadığından  
2\. kendisini doğru şekilde tanıttığından (genel kapsamlı kullanıcı aracıları kullanmadığından)  
3\. istem önbelleğe almayı optimize edebilmemiz için `x-aidev-cli-session` başlığını içerdiğinden emin olun

## Kullanım limitleri

AidevCLI Go aşağıdaki limitleri içerir:
...
AidevCLI yapılandırmanızdaki [model id](/docs/config/#models) formatı `aidev-cli-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `aidev-cli-go/kimi-k3` kullanmalısınız.
...
AidevCLI Go'yu şu amaçlarla oluşturduk:
...
4. AidevCLI ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
