...
aidev-cli'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
aidev-cli, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
AidevCLI, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

AidevCLI iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `aidev-cli.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse aidev-cli modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global aidev-cli yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
aidev-cli yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [AidevCLI Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `aidev-cli/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `aidev-cli models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/aidev-cli).
:::
