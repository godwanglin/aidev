---
title: Eklentiler
description: AidevCLI'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek AidevCLI'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya AidevCLI'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.aidev-cli/plugins/` - Proje düzeyinde eklentiler
- `~/.config/aidev-cli/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/aidev-cli/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/aidev-cli/aidev-cli.json`)
2. Project config (`aidev-cli.json`)
3. Global eklenti dizini (`~/.config/aidev-cli/plugins/`)
4. Proje eklenti dizini (`.aidev-cli/plugins/`)
...
AidevCLI bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir AidevCLI SDK istemcisi.
...
AidevCLI'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
AidevCLI masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
AidevCLI'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca AidevCLI'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, AidevCLI'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra AidevCLI'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://aidev-cli.ai/docs/sdk) bakın.
