---
title: Sunucu
description: aidev-cli sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`aidev-cli serve` komutu, aidev-cli istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `aidev-cli.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `AIDEV_CLI_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `aidev-cli` değeridir; değiştirmek isterseniz `AIDEV_CLI_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `aidev-cli serve` hem de `aidev-cli web` için geçerlidir.
...
`aidev-cli` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
aidev-cli ile programatik etkileşim için aidev-cli sunucusunu kullanın.
:::

Bu mimari, aidev-cli'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`aidev-cli serve` ile bağımsız bir sunucu başlatabilirsiniz. aidev-cli TUI açıksa, `aidev-cli serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum aidev-cli [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
aidev-cli sunucusu aşağıdaki API'leri sunar.
