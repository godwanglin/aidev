...
Ajan becerileri, aidev-cli'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
aidev-cli su konumlari tarar:

- Proje konfigurasyonu: `.aidev-cli/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/aidev-cli/skills/<name>/SKILL.md`
...
Proje yerel yollarinda aidev-cli, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.aidev-cli/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/aidev-cli/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.aidev-cli/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
aidev-cli, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`aidev-cli.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`aidev-cli.json` icinde):
