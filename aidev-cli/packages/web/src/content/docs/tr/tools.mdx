...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. aidev-cli yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
aidev-cli içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `AIDEV_CLI_EXPERIMENTAL_LSP_TOOL=true` (veya `AIDEV_CLI_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca aidev-cli provider kullanılırken veya `AIDEV_CLI_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

aidev-cli başlatırken etkinleştirmek için:
