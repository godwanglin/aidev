...
description: aidev-cli dile özgü biçimlendiriciler kullanır.
---

aidev-cli, dosyaları dile özgü formatlayıcılar kullanılarak yazıldıktan veya düzenlendikten sonra otomatik olarak formatlar. Bu, oluşturulan kodun projenizin kod stillerine uymasını sağlar.

---

## Yerleşik

aidev-cli, popüler diller ve çerçeveler için çeşitli yerleşik biçimlendiricilerle birlikte gelir. Aşağıda ihtiyaç duyduğu biçimlendiricilerin, desteklenen dosya uzantılarının ve komutların veya yapılandırma seçeneklerinin bir listesi bulunmaktadır.
...
Yani eğer projenizin `package.json` dosyasında `prettier` varsa, aidev-cli bunu otomatik olarak kullanacaktır.

---

## Nasıl çalışır?

aidev-cli bir dosyayı yazdığında veya düzenlediğinde:
...
Biçimlendiricileri aidev-cli yapılandırmanızdaki `formatter` bölümü aracılığıyla özelleştirebilirsiniz.
