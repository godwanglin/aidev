...
AidevCLI Go is a low cost **$10/month subscription** that gives you reliable access to popular open coding models.

Go works like any other provider in AidevCLI. You subscribe to AidevCLI Go and
...
AidevCLI Go gives you access to these models for **$10/month**.

---

## How it works

AidevCLI Go works like any other provider in AidevCLI.

1. You sign in to **<a href={console}>AidevCLI Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `AidevCLI Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to AidevCLI Go.
:::
...
AidevCLI Go is designed to be used with [AidevCLI](https://aidev-cli.ai) and other
...
3\. includes the `x-aidev-cli-session` header so we can optimize prompt caching

## Usage limits

AidevCLI Go includes the following limits:
...
uses the format `aidev-cli-go/<model-id>`. For example, for Kimi K3, you would
use `aidev-cli-go/kimi-k3` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with AidevCLI as well.
