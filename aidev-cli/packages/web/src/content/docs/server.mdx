---
title: Server
description: Interact with aidev-cli server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `aidev-cli serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an aidev-cli client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `aidev-cli.local` |
...
Set `AIDEV_CLI_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `aidev-cli`, or set `AIDEV_CLI_SERVER_USERNAME` to override it. This applies to both `aidev-cli serve` and `aidev-cli web`.
...
When you run `aidev-cli` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the aidev-cli server to interact with aidev-cli programmatically.
:::

This architecture lets aidev-cli support multiple clients and allows you to interact with aidev-cli programmatically.

You can run `aidev-cli serve` to start a standalone server. If you have the
aidev-cli TUI running, `aidev-cli serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the AidevCLI [IDE](/docs/ide) plugins.
...
The aidev-cli server exposes the following APIs.
