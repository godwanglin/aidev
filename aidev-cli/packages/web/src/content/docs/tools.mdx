...
Tools allow the LLM to perform actions in your codebase. AidevCLI comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in AidevCLI.
...
:::note
This tool is only available when `AIDEV_CLI_EXPERIMENTAL_LSP_TOOL=true` (or `AIDEV_CLI_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the AidevCLI or AidevCLI Go provider, or when either the `AIDEV_CLI_ENABLE_EXA` or `AIDEV_CLI_ENABLE_PARALLEL` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching AidevCLI:

```bash
AIDEV_CLI_ENABLE_EXA=1 aidev-cli
# or
AIDEV_CLI_ENABLE_PARALLEL=1 aidev-cli
```
