...
AidevCLI Go 是一项**每月 10 美元的低成本订阅服务**，让你能够稳定地访问流行的开源编程模型。

Go 的工作方式与 AidevCLI 中的任何其他提供商（provider）一样。订阅 AidevCLI Go 后你将获得 API 密钥。它是 **完全可选** 的，并非使用 AidevCLI 所必需的条件。
...
:::tip
我们测试了一组经过精选且与 AidevCLI 配合良好的模型和提供商。
:::
...
AidevCLI Go 让你能够以**每月 10 美元**的价格访问这些模型。

---

## 工作原理

AidevCLI Go 的工作方式与 AidevCLI 中的其他提供商一样。

1. 登录 **<a href={console}>AidevCLI Zen</a>**，订阅 Go，然后复制你的 API 密钥。
2. 在 TUI 中运行 `/connect` 命令，选择 `AidevCLI Go`，然后粘贴你的 API 密钥。
3. 在 TUI 中运行 `/models` 以查看通过 Go 可用的模型列表。

:::note
每个工作空间只能有一名成员订阅 AidevCLI Go。
:::
...
AidevCLI Go 适用于 [AidevCLI](https://aidev-cli.ai) 以及其他会产生类似请求的主流编程 Agent。
...
3\. 包含 `x-aidev-cli-session` 请求头，以便我们优化提示词缓存
...
你的 AidevCLI 配置中的 [模型 ID](/docs/config/#models) 使用 `aidev-cli-go/<model-id>` 格式。例如，对于 Kimi K3，你将在配置中使用 `aidev-cli-go/kimi-k3`。
