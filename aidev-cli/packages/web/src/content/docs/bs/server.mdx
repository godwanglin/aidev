---
title: Server
description: Komunicirajte s aidev-cli serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `aidev-cli serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji aidev-cli klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `aidev-cli.local` |
...
Postavite `AIDEV_CLI_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `aidev-cli`, ili postavite `AIDEV_CLI_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `aidev-cli serve` i za `aidev-cli web`.
...
Kada pokrenete `aidev-cli`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite aidev-cli server za programsku interakciju sa aidev-cli-om.
:::

Ova arhitektura omogucava aidev-cli podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `aidev-cli serve` da startate standalone server. Ako je aidev-cli TUI vec pokrenut, `aidev-cli serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste AidevCLI [IDE](/docs/ide) pluginovi.
...
aidev-cli server izlaže sljedece API-je.
