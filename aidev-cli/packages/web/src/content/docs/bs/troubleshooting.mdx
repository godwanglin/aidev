...
Da biste otklonili probleme s AidevCLI, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\aidev-cli\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `aidev-cli --log-level DEBUG`.

---

## Pohrana

AidevCLI pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/aidev-cli/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\aidev-cli`
...
AidevCLI Desktop pokreće lokalni AidevCLI server (`aidev-cli-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `AidevCLI` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/aidev-cli/aidev-cli.jsonc` (ili `~/.config/aidev-cli/aidev-cli.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/aidev-cli/aidev-cli.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\aidev-cli\aidev-cli.jsonc`
...
AidevCLI također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/aidev-cli/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\aidev-cli\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga AidevCLI mogao ponovo izgraditi.

1. Potpuno zatvorite AidevCLI Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/aidev-cli`
- **Linux**: obrišite `~/.cache/aidev-cli` (ili pokrenite `rm -rf ~/.cache/aidev-cli`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\aidev-cli`

3. Ponovo pokrenite AidevCLI Desktop.

---

### Rješavanje problema sa vezom na serveru

AidevCLI Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `aidev-cli.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `AIDEV_CLI_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `AIDEV_CLI_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, AidevCLI Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa AidevCLI karakteristikama.

---

### Obavještenja se ne prikazuju

AidevCLI Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za AidevCLI u postavkama vašeg OS-a, i
...
1. Zatvorite AidevCLI Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka AidevCLI Desktop aplikacije):

- `aidev-cli.settings.dat` (URL zadanog servera za desktop)
- `aidev-cli.global.dat` i `aidev-cli.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `aidev-cli models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\aidev-cli`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. AidevCLI dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\aidev-cli`

2. Ponovo pokrenite AidevCLI da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti AidevCLI da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
AidevCLI će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
