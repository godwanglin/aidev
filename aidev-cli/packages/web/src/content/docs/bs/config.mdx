---
title: Konfiguracija
description: Korištenje AidevCLI JSON konfiguracije.
---

Možete konfigurirati AidevCLI koristeći JSON konfiguracijski fajl.

---

## Format

AidevCLI podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/aidev-cli`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/aidev-cli/aidev-cli.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`AIDEV_CLI_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`aidev-cli.json` u projektu) - postavke specifične za projekat
5. **`.aidev-cli` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`AIDEV_CLI_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.aidev-cli` i `~/.config/aidev-cli` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/aidev-cli` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu AidevCLI konfiguraciju u `~/.config/aidev-cli/aidev-cli.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/aidev-cli/tui.json`.
...
Dodajte `aidev-cli.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se AidevCLI pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `AIDEV_CLI_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `AIDEV_CLI_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.aidev-cli` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.aidev-cli`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`aidev-cli.ai/config.json`**](https://aidev-cli.ai/config.json).

TUI konfiguracija koristi [**`aidev-cli.ai/tui.json`**](https://aidev-cli.ai/tui.json).
...
Koristite `AIDEV_CLI_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `aidev-cli.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `aidev-cli serve` i `aidev-cli web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš AidevCLI server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `aidev-cli.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj AidevCLI konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, AidevCLI pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/aidev-cli/agents/` ili `.aidev-cli/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, AidevCLI će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`aidev-cli run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/aidev-cli/commands/` ili `.aidev-cli/commands/`. [Saznajte više ovdje](/docs/commands).
...
AidevCLI će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, AidevCLI **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju AidevCLI sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.aidev-cli/plugins/` ili `~/.config/aidev-cli/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite AidevCLI da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
