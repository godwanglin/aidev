---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u AidevCLI.
---

import config from "../../../../config.mjs"
export const console = config.console

AidevCLI koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj AidevCLI konfiguraciji.
...
u `~/.local/share/aidev-cli/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem AidevCLI
config.
...
AidevCLI Zen je lista modela koje je obezbedio AidevCLI tim koji su bili
testirano i potvrđeno da dobro radi sa AidevCLI. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa AidevCLI Zen.
:::
...
Radi kao i svaki drugi provajder u AidevCLI i potpuno je opcionalan za korištenje.

---

## AidevCLI Go

AidevCLI Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža AidevCLI tim i koji su testirani i verificirani da dobro rade s AidevCLI-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `AidevCLI Go` i idite na [aidev-cli.ai/auth](https://aidev-cli.ai/zen).
...
Da biste koristili Amazon Bedrock s AidevCLI:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete aidev-cli:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `aidev-cli.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u AidevCLI.
:::
...
Možete konfigurirati aidev-cli za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi aidev-cli ispravno radio.
   :::
...
Također možete dodati modele kroz svoju aidev-cli konfiguraciju.
...
da pohrani token u aidev-cli auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
AidevCLI koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate AidevCLI
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`aidev-cli.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [aidev-cli-gitlab-auth](https://www.npmjs.com/package/aidev-cli-gitlab-auth).

##### Konfiguracija

Prilagodite putem `aidev-cli.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s aidev-cli:
...
Za korištenje Google Vertex AI s AidevCLI:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće aidev-cli.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem aidev-clia, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [aidev-cli-helicone-session](https://github.com/H2Shami/aidev-cli-helicone-session) da automatski prijavite svaki AidevCLI razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki AidevCLI razgovor naveden kao zasebna sesija.
...
Možete konfigurirati aidev-cli za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati aidev-cli za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati aidev-cli za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za AidevCLI. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/aidev-cli) za detalje.
:::
...
Da biste koristili Ollama Cloud s AidevCLI:
...
3. Kopirajte API ključ za korištenje u AidevCLI.
...
6. **Važno**: Prije upotrebe modela oblaka u AidevCLI, morate lokalno povući informacije o modelu:
...
AidevCLI Zen je lista testiranih i verifikovanih modela koju je obezbedio AidevCLI tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>AidevCLI Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **AidevCLI Zen**.

```txt
   /connect
```

3. Unesite svoj AidevCLI API ključ.
...
Također možete dodati dodatne modele putem vaše aidev-cli konfiguracije.
...
5. Također ih možete prilagoditi putem vaše aidev-cli konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju aidev-cli konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s aidev-cli-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `aidev-cli.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju AidevCLI da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `aidev-cli auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite AidevCLI konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj aidev-cli konfiguraciji.
