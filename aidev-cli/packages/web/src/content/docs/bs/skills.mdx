...
Agent skills omogucavaju AidevCLI da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
AidevCLI pretrazuje ove lokacije:

- Konfiguracija projekta: `.aidev-cli/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/aidev-cli/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, AidevCLI ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.aidev-cli/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/aidev-cli/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.aidev-cli/skills/git-release/SKILL.md` ovako:
...
AidevCLI navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `aidev-cli.json`:
...
**Za ugradene agente** (u `aidev-cli.json`):
