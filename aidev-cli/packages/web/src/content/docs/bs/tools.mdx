...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. AidevCLI dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u AidevCLI.
...
:::note
Ovaj alat je dostupan samo kada je `AIDEV_CLI_EXPERIMENTAL_LSP_TOOL=true` (ili `AIDEV_CLI_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz AidevCLI provajdera ili kada je varijabla `AIDEV_CLI_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju AidevCLI:
