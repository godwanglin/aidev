...
AidevCLI Go je povoljna pretplata od **$10/mjesečno** koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u AidevCLI-u. Pretplatite se na AidevCLI Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili AidevCLI.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa AidevCLI-om.
:::
...
AidevCLI Go vam daje pristup ovim modelima za **$10/mjesečno**.

---

## Kako funkcioniše

AidevCLI Go radi kao bilo koji drugi provajder u AidevCLI-u.

1. Prijavite se na **<a href={console}>AidevCLI Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `AidevCLI Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na AidevCLI Go.
:::
...
AidevCLI Go je osmišljen za korištenje s [AidevCLI-om](https://aidev-cli.ai) i drugim popularnim agentima za programiranje koji generišu slične vrste zahtjeva.

Saobraćaj se nadzire radi otkrivanja zloupotrebe koja narušava iskustvo drugih korisnika.

Kako vaš račun ne bi bio označen, pobrinite se da alat koji koristite

1\. ne generiše saobraćaj koji predstavlja zloupotrebu  
2\. se ispravno identifikuje (bez generičkih User-Agent identifikatora)  
3\. uključuje zaglavlje `x-aidev-cli-session` kako bismo mogli optimizovati keširanje promptova

## Ograničenja upotrebe

AidevCLI Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj AidevCLI konfiguraciji
koristi format `aidev-cli-go/<model-id>`. Na primjer, za Kimi K3, koristili biste
`aidev-cli-go/kimi-k3` u svojoj konfiguraciji.
...
Napravili smo AidevCLI Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz AidevCLI koristite i bilo kojeg drugog provajdera.
