---
title: Dodaci
description: Napišite vlastite dodatke za proširenje AidevCLI.
---

Dodaci vam omogućavaju da proširite AidevCLI spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja AidevCLI.
...
- `.aidev-cli/plugins/` - Dodaci na nivou projekta
- `~/.config/aidev-cli/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/aidev-cli/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/aidev-cli/aidev-cli.json`)
2. Konfiguracija projekta (`aidev-cli.json`)
3. Globalni direktorij dodataka (`~/.config/aidev-cli/plugins/`)
4. Direktorij dodataka projekta (`.aidev-cli/plugins/`)
...
AidevCLI pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje AidevCLI.
...
Spriječite aidev-cli da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u aidev-cli:
...
Pomoćnik `tool` kreira prilagođeni alat koji aidev-cli može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za aidev-cli zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://aidev-cli.ai/docs/sdk) za detalje.
