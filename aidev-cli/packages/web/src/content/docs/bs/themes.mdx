...
U AidevCLI mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, AidevCLI koristi nasu `aidev-cli` temu.
...
AidevCLI dolazi sa vise ugradenih tema.
...
- Zele da AidevCLI odgovara izgledu njihovog terminala
...
AidevCLI podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/aidev-cli/themes/*.json` ili `$XDG_CONFIG_HOME/aidev-cli/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.aidev-cli/themes/*.json`
4. **Trenutni radni direktorij** - `./.aidev-cli/themes/*.json`
