...
Možete dodati vanjske alate u AidevCLI koristeći _Model Context Protocol_, ili MCP. AidevCLI podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [AidevCLI Config](https://aidev-cli.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/aidev-cli`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
AidevCLI automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, AidevCLI će:
...
Ako server zahtijeva autentifikaciju, AidevCLI će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `aidev-cli mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, AidevCLI sigurno čuva tokene u `~/.local/share/aidev-cli/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u AidevCLI, zajedno s ugrađenim alatima. Možete njima upravljati kroz AidevCLI konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete AidevCLI sa vašim Sentry nalogom.
