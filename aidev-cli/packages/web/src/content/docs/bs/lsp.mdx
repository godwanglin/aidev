---
title: LSP serveri
description: AidevCLI se integriše sa vašim LSP serverima.
---

AidevCLI se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

AidevCLI dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `AIDEV_CLI_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i aidev-cli otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj aidev-cli konfiguraciji.
