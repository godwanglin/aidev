---
title: Serveur
description: Interagissez avec le serveur aidev-cli via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `aidev-cli serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client aidev-cli peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `aidev-cli.local` |
...
Définissez `AIDEV_CLI_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `aidev-cli`, ou définissez `AIDEV_CLI_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `aidev-cli serve` et à `aidev-cli web`.
...
Lorsque vous exécutez `aidev-cli`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur aidev-cli pour interagir avec aidev-cli par programmation.
:::

Cette architecture permet à aidev-cli de prendre en charge plusieurs clients et vous permet d'interagir avec aidev-cli par programme.

Vous pouvez exécuter `aidev-cli serve` pour démarrer un serveur autonome. Si vous avez le
aidev-cli TUI en cours d'exécution, `aidev-cli serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins AidevCLI [IDE](/docs/ide).
...
Le serveur aidev-cli expose les API suivantes.
