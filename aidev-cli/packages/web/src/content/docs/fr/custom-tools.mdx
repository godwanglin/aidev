...
description: Créez des outils que le LLM peut appeler dans aidev-cli.
---

Les outils personnalisés sont des fonctions que vous créez et que le LLM peut appeler pendant les conversations. Ils fonctionnent avec les [outils intégrés](/docs/tools) de aidev-cli comme `read`, `write` et `bash`.
...
- Localement en les plaçant dans le répertoire `.aidev-cli/tools/` de votre projet.
- Ou globalement, en les plaçant dans `~/.config/aidev-cli/tools/`.
