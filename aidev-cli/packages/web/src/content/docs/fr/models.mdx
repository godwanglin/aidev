...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez AidevCLI.
...
Voici plusieurs modèles qui fonctionnent bien avec AidevCLI, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration AidevCLI.

```json title="aidev-cli.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [AidevCLI Zen](/docs/zen), vous utiliserez `aidev-cli/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="aidev-cli.jsonc" {7-12,19-24}
{
  "$schema": "https://aidev-cli.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="aidev-cli.jsonc" {6-21}
{
  "$schema": "https://aidev-cli.ai/config.json",
  "provider": {
    "aidev-cli": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. AidevCLI est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

AidevCLI est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="aidev-cli.jsonc" {7-18}
{
  "$schema": "https://aidev-cli.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque AidevCLI démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration AidevCLI.
