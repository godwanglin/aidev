...
Avec AidevCLI, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, AidevCLI utilise notre propre thème `aidev-cli`.
...
AidevCLI est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que AidevCLI corresponde à l'apparence de leur terminal
...
AidevCLI prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/aidev-cli/themes/*.json` ou `$XDG_CONFIG_HOME/aidev-cli/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.aidev-cli/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.aidev-cli/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/aidev-cli/themes
vim ~/.config/aidev-cli/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .aidev-cli/themes
vim .aidev-cli/themes/my-theme.json
```
...
{
  "$schema": "https://aidev-cli.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
