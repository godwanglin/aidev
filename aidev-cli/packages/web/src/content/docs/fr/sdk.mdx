---
title: SDK
description: Client JS de type sécurisé pour le serveur aidev-cli.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK aidev-cli JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler aidev-cli par programme.
...
Créez une instance de aidev-cli :
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `aidev-cli.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createOpencode } from "@aidev-cli/sdk"

const aidev-cli = await createOpencode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${aidev-cli.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de aidev-cli, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@aidev-cli/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*aidev-cli" },
})
