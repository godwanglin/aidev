---
title: Plugins
description: Écrivez vos propres plugins pour étendre AidevCLI.
---

Les plugins vous permettent d'étendre AidevCLI en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de AidevCLI.
...
- `.aidev-cli/plugins/` - Plugins au niveau du projet
- `~/.config/aidev-cli/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/aidev-cli/node_modules/`.
...
1. Configuration globale (`~/.config/aidev-cli/aidev-cli.json`)
2. Configuration du projet (`aidev-cli.json`)
3. Répertoire global des plugins (`~/.config/aidev-cli/plugins/`)
4. Répertoire des plugins du projet (`.aidev-cli/plugins/`)
...
AidevCLI exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK aidev-cli pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre aidev-cli.
...
:::note
Si vous utilisez l'application de bureau AidevCLI, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez aidev-cli de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à aidev-cli :
...
L'assistant `tool` crée un outil personnalisé que aidev-cli peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour aidev-cli aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://aidev-cli.ai/docs/sdk) pour plus de détails.
