...
Create `.aidev-cli/commands/test.md`:
...
You can add custom commands through the AidevCLI config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your AidevCLI [config](/docs/config):
...
- Per-project: `.aidev-cli/commands/`
...
aidev-cli includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
