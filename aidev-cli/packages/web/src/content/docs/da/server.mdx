---
title: Server
description: Interager med aidev-cli-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `aidev-cli serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en aidev-cli-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `aidev-cli.local` |
...
Indstil `AIDEV_CLI_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `aidev-cli`, eller indstil `AIDEV_CLI_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `aidev-cli serve` og `aidev-cli web`.
...
Når du kører `aidev-cli` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug aidev-cli-serveren til at interagere med aidev-cli programmatisk.
:::

Denne arkitektur lader aidev-cli understøtte flere klienter og lader dig interagere med aidev-cli programmatisk.

Du kan køre `aidev-cli serve` for at starte en selvstændig server. Hvis du har
aidev-cli TUI kørende, vil `aidev-cli serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af AidevCLI [IDE](/docs/ide) plugins.
...
AidevCLI-serveren viser følgende API'er.
