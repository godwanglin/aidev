...
AidevCLI Go er et lavprisabonnement til **$10/måned**, der giver dig pålidelig adgang til populære åbne kodningsmodeller.

Go fungerer som enhver anden udbyder i AidevCLI. Du abonnerer på AidevCLI Go og
...
:::tip
Vi testede en udvalgt gruppe af modeller og udbydere, der fungerer godt med AidevCLI.
:::
...
AidevCLI Go giver dig adgang til disse modeller for **$10/måned**.

---

## Sådan fungerer det

AidevCLI Go fungerer som enhver anden udbyder i AidevCLI.

1. Du logger ind på **<a href={console}>AidevCLI Zen</a>**, abonnerer på Go, og
   kopierer din API-nøgle.
2. Du kører kommandoen `/connect` i TUI'en, vælger `AidevCLI Go`, og indsætter
   din API-nøgle.
3. Kør `/models` i TUI'en for at se listen over tilgængelige modeller gennem Go.

:::note
Kun ét medlem per arbejdsområde kan abonnere på AidevCLI Go.
:::
...
AidevCLI Go er designet til brug med [AidevCLI](https://aidev-cli.ai) og andre populære kodningsagenter, der genererer lignende typer anmodninger.

Trafikken overvåges for misbrug, der forringer oplevelsen for andre brugere.

For at sikre, at din konto ikke bliver markeret, skal du sørge for, at det værktøj, du bruger,

1\. ikke genererer misbrugstrafik  
2\. identificerer sig korrekt (ingen generiske User-Agent-identifikatorer)  
3\. inkluderer `x-aidev-cli-session`-headeren, så vi kan optimere prompt-caching

## Forbrugsgrænser

AidevCLI Go inkluderer følgende grænser:
...
Dit [model id](/docs/config/#models) i din AidevCLI config
bruger formatet `aidev-cli-go/<model-id>`. For eksempel for Kimi K3, vil du
bruge `aidev-cli-go/kimi-k3` i din config.
...
4. Undgå **lock-in** ved også at lade dig bruge enhver anden udbyder med AidevCLI.
