...
Her er alle de innebygde verktøyene som er tilgængelige i AidevCLI.
...
:::note
Dette verktøyet er bare tilgængelig når `AIDEV_CLI_EXPERIMENTAL_LSP_TOOL=true` (eller `AIDEV_CLI_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger AidevCLI-leverandøren eller når miljøvariabelen `AIDEV_CLI_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
