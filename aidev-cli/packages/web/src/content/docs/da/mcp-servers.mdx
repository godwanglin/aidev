...
Du kan tilføje eksterne værktøjer til AidevCLI ved hjælp af _Model Context Protocol_ eller MCP. AidevCLI understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [AidevCLI Config](https://aidev-cli.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/aidev-cli`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
AidevCLI håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil AidevCLI:
...
Hvis serveren kræver godkendelse, vil AidevCLI bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `aidev-cli mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil AidevCLI opbevare tokens sikkert i `~/.local/share/aidev-cli/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem AidevCLI-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde AidevCLI til din Sentry-konto.
