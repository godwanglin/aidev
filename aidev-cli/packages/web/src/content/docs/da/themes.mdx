...
Med AidevCLI kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger AidevCLI vårt eget `aidev-cli`-tema.
...
AidevCLI kommer med flere innebygde temaer.
...
- Vil at AidevCLI skal matche terminalens utseende
...
AidevCLI støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/aidev-cli/themes/*.json` eller `$XDG_CONFIG_HOME/aidev-cli/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.aidev-cli/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.aidev-cli/themes/*.json`
