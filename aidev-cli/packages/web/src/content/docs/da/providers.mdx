---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i AidevCLI.
---

import config from "../../../../config.mjs"
export const console = config.console

AidevCLI bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i AidevCLI-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/aidev-cli/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i AidevCLI-konfigurationen.
...
AidevCLI Zen er en liste over modeller leveret af AidevCLI-teamet, som er blevet testet og verificeret til at fungere godt med AidevCLI. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med AidevCLI Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg aidev-cli og gå til [aidev-cli.ai/auth](https://aidev-cli.ai/auth).
...
Det fungerer som alle andre udbydere i AidevCLI og er helt valgfrit at bruge.

---

## AidevCLI Go

AidevCLI Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af AidevCLI-teamet, som er testet og verificeret til at fungere godt med AidevCLI.

1. Kør kommandoen `/connect` i TUI, vælg `AidevCLI Go`, og gå til [aidev-cli.ai/auth](https://aidev-cli.ai/zen).
...
Sådan bruger du Amazon Bedrock med AidevCLI:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører aidev-cli:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `aidev-cli.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i AidevCLI understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere aidev-cli til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at aidev-cli skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem aidev-cli-konfigurationen.
...
gemme tokenet i aidev-cli auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
AidevCLI bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse AidevCLI
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`aidev-cli.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [aidev-cli-gitlab-auth](https://www.npmjs.com/package/aidev-cli-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `aidev-cli.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med AidevCLI:
...
Sådan bruger du Google Vertex AI med AidevCLI:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører aidev-cli.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem aidev-cli, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [aidev-cli-helicone-session](https://github.com/H2Shami/aidev-cli-helicone-session) plugin for automatisk at logge hver AidevCLI-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver AidevCLI-samtale opført som en separat session.
...
Du kan konfigurere aidev-cli til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere aidev-cli til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere aidev-cli til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for AidevCLI. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/aidev-cli) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med AidevCLI:
...
3. Kopier API-nøglen til brug i AidevCLI.
...
6. **Vigtigt**: Før du bruger skymodeller i AidevCLI, skal du hente modelinformationen lokalt:
...
AidevCLI Zen er en liste over testede og verificerede modeller leveret af AidevCLI-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>AidevCLI Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **AidevCLI Zen**.

   ```txt
   /connect
   ```

3. Indtast AidevCLI API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem aidev-cli-konfigurationen.
...
5. Du kan også tilpasse dem gennem aidev-cli-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med AidevCLI:
...
Du kan også tilpasse modeller gennem aidev-cli-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med aidev-cli. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `aidev-cli.json`-filen i projektkataloget:
...
`limit`-felterne lader AidevCLI forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `aidev-cli auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek aidev-cli-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i aidev-cli-konfigurationen.
