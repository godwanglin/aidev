---
title: Plugins
description: Skriv dine egne plugins for at udvide AidevCLI.
---

Plugins giver dig mulighed for at udvide AidevCLI ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre AidevCLIs standardadfærd.
...
- `.aidev-cli/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/aidev-cli/node_modules/`.
...
1. Global konfiguration (`~/.config/aidev-cli/aidev-cli.json`)
2. Projektkonfiguration (`aidev-cli.json`)
3. Global plugin-mappe (`~/.config/aidev-cli/plugins/`)
4. Projekt plugin bibliotek (`.aidev-cli/plugins/`)
...
AidevCLI kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En aidev-cli SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide aidev-cli.
...
:::note
Hvis du bruger AidevCLI desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå aidev-cli i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til aidev-cli:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som aidev-cli kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for aidev-cli sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://aidev-cli.ai/docs/sdk) for detaljer.
