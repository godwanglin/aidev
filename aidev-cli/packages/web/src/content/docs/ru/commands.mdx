...
Создайте `.aidev-cli/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию aidev-cli или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем aidev-cli [config](/docs/config):
...
- Глобальный: `~/.config/aidev-cli/commands/`
- Для каждого проекта: `.aidev-cli/commands/`
...
aidev-cli включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
