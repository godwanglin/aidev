...
import { and, Database, eq, isNull, lt, or, sql } from "@aidev-cli/console-core/drizzle/index.js"
import { KeyTable } from "@aidev-cli/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@aidev-cli/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@aidev-cli/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@aidev-cli/console-core/util/date.js"
import { Identifier } from "@aidev-cli/console-core/identifier.js"
import { Billing } from "@aidev-cli/console-core/billing.js"
import { Actor } from "@aidev-cli/console-core/actor.js"
import { WorkspaceTable } from "@aidev-cli/console-core/schema/workspace.sql.js"
import { ZenData } from "@aidev-cli/console-core/model.js"
import { Subscription } from "@aidev-cli/console-core/subscription.js"
import { BlackData } from "@aidev-cli/console-core/black.js"
import { UserTable } from "@aidev-cli/console-core/schema/user.sql.js"
import { ModelTable } from "@aidev-cli/console-core/schema/model.sql.js"
import { ProviderTable } from "@aidev-cli/console-core/schema/provider.sql.js"
...
import { LiteData } from "@aidev-cli/console-core/lite.js"
import { Resource } from "@aidev-cli/console-resource"
...
import { Workspace } from "@aidev-cli/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-aidev-cli-session") ?? ""
    const requestId = input.request.headers.get("x-aidev-cli-request") ?? ""
    const ocClient = input.request.headers.get("x-aidevCliClient") ?? ""
    const projectId = input.request.headers.get("x-aidev-cli-project") ?? ""
    ...
    if (authInfo && opts.modelList === "lite" && requiresGoTrainingConsent(modelInfo.id) && !authInfo.allowTraining)
      throw new DataPolicyError(
        t("zen.api.error.trainingNotAllowed", {
          consoleGoUrl: `https://aidev_cli.ai/workspace/${authInfo.workspaceID}/go`,
        }),
      )
    ...
    const providerRequest = async () => {
      ...
      const res = await fetch(reqUrl, {
        ...
        headers: (() => {
          ...
          if (!isNewInference) {
            headers.delete("x-aidev-cli-session")
            headers.delete("x-aidev-cli-project")
            headers.delete("x-aidevCliClient")
            headers.delete("x-aidev-cli-request")
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-aidev-cli-endpoint-id")
        const resEndpointModelId = res.headers.get("x-aidev-cli-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      ...
      try {
        const consoleGoUrl = `https://aidev_cli.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://aidev_cli.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://aidev_cli.ai/workspace/${authInfo.workspaceID}/members`
