...
import { Database, eq } from "@aidev-cli/console-core/drizzle/index.js"
...
export async function proxyInference(request: Request, clientIP?: string): Promise<Response | undefined> {
  ...
  const requestID = request.headers.get("x-aidev-cli-request-id") ?? request.headers.get("x-aidev-cli-request")
  if (requestID) forwarded.headers.set("x-aidev-cli-request-id", requestID)
