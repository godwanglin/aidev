...
import { Resource, waitUntil } from "@aidev-cli/console-resource"
import { LOCALE_HEADER, cookie, localeFromRequest, route, tag } from "~/lib/language"

const dataPath = "/data"
const statsCacheParam = "__aidevCli_stats_locale"
...
export async function statsProxy(evt: APIEvent) {
  ...
  targetUrl.hostname = Resource.App.stage === "production" ? "stats.aidev_cli.ai" : "stats.dev.aidev_cli.ai"
  targetUrl.port = ""
...
function withStatsCacheStatus(response: Response, status: "HIT" | "MISS") {
  const headers = new Headers(response.headers)
  headers.set("x-aidev-cli-stats-cache", status)
