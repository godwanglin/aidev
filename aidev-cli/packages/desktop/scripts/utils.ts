...
export function resolveChannel(): Channel {
  const raw = Bun.env.AIDEV_CLI_CHANNEL
...
export const CLI_BINARIES: Array<{ rustTarget: string; package: string; os: string; cpu: string }> = [
  {
    ...
    package: "@aidev-cli/cli-darwin-arm64",
    os: "darwin",
    cpu: "arm64",
  },
  {
    rustTarget: "x86_64-apple-darwin",
    package: "@aidev-cli/cli-darwin-x64-baseline",
    os: "darwin",
    cpu: "x64",
  },
  {
    rustTarget: "aarch64-pc-windows-msvc",
    package: "@aidev-cli/cli-windows-arm64",
    os: "win32",
    cpu: "arm64",
  },
  {
    rustTarget: "x86_64-pc-windows-msvc",
    package: "@aidev-cli/cli-windows-x64-baseline",
    os: "win32",
    cpu: "x64",
  },
  {
    rustTarget: "x86_64-unknown-linux-gnu",
    package: "@aidev-cli/cli-linux-x64-baseline",
    os: "linux",
    cpu: "x64",
  },
  {
    rustTarget: "aarch64-unknown-linux-gnu",
    package: "@aidev-cli/cli-linux-arm64",
    os: "linux",
    cpu: "arm64",
  },
]
...
export async function downloadCliToResources() {
  ...
  const directory = await mkdtemp(join(tmpdir(), "aidev-cli-cli-"))
  const dest = windowsify("resources/aidev-cli-cli")
  try {
    await $`bun install --no-save --cwd ${directory} ${`${cli.package}@${CLI_VERSION}`} ${`--os=${cli.os}`} ${`--cpu=${cli.cpu}`}`
    await copyFile(
      join(directory, "node_modules", cli.package, "bin", cli.os === "win32" ? "aidevCli2.exe" : "aidevCli2"),
      dest,
    )
  } finally {
