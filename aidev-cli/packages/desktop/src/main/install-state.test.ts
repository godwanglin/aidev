...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier AidevCLI launch", () => {
    expect(hasExistingAppState([file("aidev_cli.settings")])).toBe(true)
    expect(hasExistingAppState([file("aidev_cli.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("aidev-cli")])).toBe(true)
  })
})
