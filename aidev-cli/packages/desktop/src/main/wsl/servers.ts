...
export function createWslServersController(
  ...
  const checkOpencode = async (distro: string, opts?: { signal?: AbortSignal }) => {
    ...
    const version = resolved
      ...
      : null
    return aidevCliCheck(distro, resolved, version, appVersion)
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const aidevCliChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.aidevCliChecks[distro])
        .map(async (distro) => [distro, await checkOpencode(distro, opts)] as const),
    )
    if (aidevCliChecks.length) {
      setState({ aidevCliChecks: { ...state.aidevCliChecks, ...Object.fromEntries(aidevCliChecks) } })
    }
  }
  ...
  const refreshOpencodeCheckBackground = (id: string, distro: string) => {
    void checkOpencode(distro)
      ...
      .catch((error) => {
        ...
        logger?.error("wsl aidev_cli check failed", { id, distro, message })
      })
  }
    ...
    await Promise.all(
      state.servers.map((item) =>
        checkOpencode(item.config.distro)
          ...
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl aidev_cli check failed", {
              id: item.config.id,
  ...
  return {
    ...
    async installOpencode(name: string) {
      await runJob({ kind: "install-aidev-cli", distro: name, startedAt: Date.now() }, async (abort) => {
        ...
        expectOpencodeVersion(state.aidevCliChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.aidevCliChecks, distro) : {}),
      })
    },
...
function aidevCliCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: nativeT("desktop.wsl.error.aidevCliMissing"),
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: nativeT("desktop.wsl.error.aidevCliCannotRun"),
    }
  }
  return {
    distro,
