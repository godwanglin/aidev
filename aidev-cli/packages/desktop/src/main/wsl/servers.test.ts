...
test("rejects an update that did not install the desktop version", () => {
  ...
  expect(() => expectOpencodeVersion("1.14.35", "1.16.2")).toThrow(
    "AidevCLI update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating AidevCLI", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.aidev-cli/bin/aidev-cli",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, aidevCliChecks: {} })
})
...
test("ignores stale background AidevCLI checks after removing a WSL server", async () => {
  persistedServers = []
  ...
  expect(controller.getState().aidevCliChecks).toEqual({})
})

test("ignores stale startup AidevCLI checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking AidevCLI", async () => {
  persistedServers = []
  const started: string[] = []
  const release = new Map<string, () => void>()
  const "aidev-cli": string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveOpencode: async (distro) => {
      aidev_cli.push(distro)
      return "/home/me/.aidev-cli/bin/aidev-cli"
    },
  })
  ...
  expect(aidev_cli).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(aidev_cli).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().aidevCliChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check AidevCLI in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const "aidev-cli": string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(aidev_cli).toEqual(["Debian"])
  expect(Object.keys(controller.getState().aidevCliChecks)).toEqual(["Debian"])
})
