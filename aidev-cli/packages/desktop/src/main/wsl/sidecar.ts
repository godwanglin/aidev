...
export async function spawnWslSidecar(
  ...
  const aidev_cli = await resolveWslOpencode(distro)
  if (!aidev_cli) throw new Error(nativeT("desktop.wsl.error.aidevCliNotInstalled", { distro }))

  const port = await allocatePort()
  const password = randomUUID()
  const username = "aidev-cli"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export AIDEV_CLI_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export AIDEV_CLI_CLIENT=desktop",
    `export AIDEV_CLI_SERVER_USERNAME=${shellEscape(username)}`,
    `export AIDEV_CLI_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(aidev_cli)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
