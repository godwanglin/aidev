...
const TEST_ONBOARDING = process.env.AIDEV_CLI_TEST_ONBOARDING === "1"
const SIDECAR_VERSION = process.env.AIDEV_CLI_SIDECAR_V2 === "1" ? "v2" : "v1"
...
const main = Effect.gen(function* () {
  ...
  process.env.AIDEV_CLI_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.aidev_cli.desktop.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `aidev-cli-onboarding-${randomUUID()}`)
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "AidevCLI Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith(""aidev-cli"://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("aidev-cli")
  registerRendererProtocol()
  setDockIcon()
  ...
  const loadingTask = yield* Effect.gen(function* () {
    ...
    const port = yield* Effect.gen(function* () {
      const fromEnv = process.env.AIDEV_CLI_PORT
      if (fromEnv) {
