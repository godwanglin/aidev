...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.aidev_cli.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `aidev_cli.settings.dat` is special: it maps to the `aidev_cli.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"aidev_cli.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // aidev_cli.settings.dat → the electron settings store ("aidev_cli.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "aidev_cli.global.dat").
  const storeName = filename === "aidev_cli.settings.dat" ? "aidev_cli.settings" : filename
