...
import type { UpdaterState } from "@aidev-cli/app/updater"
...
import "./styles.css"
import { Splash } from "@aidev-cli/ui/logo"
import { useTheme } from "@aidev-cli/ui/theme/context"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.AIDEV_CLI_CHANNEL === "prod" &&
...
const deepLinkEvent = ""aidev-cli":deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__AIDEV_CLI__ ??= {}
  const pending = window.__AIDEV_CLI__.deepLinks ?? []
  window.__AIDEV_CLI__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `aidev_cli.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, onClick) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://aidev_cli.ai/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("aidev_cli.global.dat").getItem("language")
