...
> current private `@aidev-cli/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@aidev-cli/ai`
...
- Secondary audience: AidevCLI and other durable agent runtimes
...
import { LLM } from "@aidev-cli/ai"
import { OpenAI } from "@aidev-cli/ai/providers/openai"
...
```ts
import { Message } from "@aidev-cli/ai"
...
import { LLM, Tool } from "@aidev-cli/ai"
...
AidevCLI and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@aidev-cli/ai/promise"
import { OpenAI } from "@aidev-cli/ai/providers/openai"
...
```ts
import { LLMSchema } from "@aidev-cli/ai/schema"
...
```ts
import { Provider, Protocol } from "@aidev-cli/ai/provider"
...
@aidev-cli/ai/promise
  Promise/AsyncIterable LLM facade

@aidev-cli/ai/schema
  serializable domain schemas

@aidev-cli/ai/provider
  experimental Provider and Protocol authoring APIs

@aidev-cli/ai/providers/openai
@aidev-cli/ai/providers/anthropic
@aidev-cli/ai/providers/google
...
```
...
AidevCLI should migrate to `generateTurn` / `streamTurn`, preserving its durable
