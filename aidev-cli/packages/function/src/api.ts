...
export default new Hono<{ Bindings: Env }>()
  ...
  .post("/exchange_github_app_token", async (c) => {
    const EXPECTED_AUDIENCE = "aidev-cli-github-action"
  ...
  /**
   * Used by the GitHub action to get GitHub installation access token given user PAT token (used when testing `aidev-cli github run` locally)
   */
  ...
  /**
   * Used by the aidev_cli CLI to check if the GitHub app is installed
   */
