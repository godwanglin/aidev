import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@aidev-cli/plugin/tui"
...
import type { useAidevKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useAidevKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getaidevCliModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getaidevCliModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
