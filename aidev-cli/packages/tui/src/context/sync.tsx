    ...
    event.subscribe((event, { directory, workspace }) => {
      switch (event.type) {
        ...
        case "message.part.updated": {
          ...
          setStore("part", event.properties.part.messageID, (prev) => {
            const next = Array.isArray(prev) ? [...prev] : []
            next.splice(result.index, 0, event.properties.part)
            return next
          })
          break
        }
        ...
        case "message.part.delta": {
          ...
          if (!parts || !Array.isArray(parts)) break
          const result = search(parts, event.properties.partID, (part) => part.id)
          if (!result.found) break
          touchPart(event.properties.sessionID, event.properties.partID)
          const targetPart = parts[result.index]
          if (targetPart && typeof targetPart === "object") {
            const field = event.properties.field as keyof typeof targetPart
            const existing = String(targetPart[field] ?? "")
            setStore("part", event.properties.messageID, result.index, field, (existing + event.properties.delta) as any)
          }
          break
        }
