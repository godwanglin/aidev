...
export const AIDEV_CLI_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const AIDEV_CLI_MODE_KEY = "aidev_cli.mode"

export const AidevKeymapProvider = KeymapProvider
export const useAidevKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type aidevCliModeStack = ReturnType<typeof createaidevCliModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, aidevCliModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createaidevCliModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(AIDEV_CLI_MODE_KEY, AIDEV_CLI_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(AIDEV_CLI_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(AIDEV_CLI_MODE_KEY, stack.at(-1)?.mode ?? AIDEV_CLI_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? AIDEV_CLI_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(AIDEV_CLI_MODE_KEY, undefined)
...
export function useaidevCliModeStack() {
  return getaidevCliModeStack(useAidevKeymap())
}

export function getaidevCliModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("aidev-cli mode stack is not registered for this keymap")
  return value
}
...
export function registerAidevKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createaidevCliModeStack(keymap)
