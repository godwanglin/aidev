...
import { getaidevCliModeStack, AIDEV_CLI_BASE_MODE, AidevKeymapProvider, registerAidevKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerAidevKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when aidev_cli mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getaidevCliModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getaidevCliModeStack(keymap).push("autocomplete")
