...
- `aidev-cli dev web` proxies `https://app.aidev-cli.ai`, so local UI/CSS changes will not show there.
- For local UI changes, run the backend and app dev servers separately.
- Backend (from `packages/aidev-cli`): `bun run ./src/index.ts serve --port 4096`
