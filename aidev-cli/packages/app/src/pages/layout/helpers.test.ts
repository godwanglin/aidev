...
import { type Session } from "@aidev-cli/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  compareSessionTime,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink(""aidev-cli"://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink(""aidev-cli"://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink(""aidev-cli"://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink(""aidev-cli"://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink(""aidev-cli"://open-project")).toBeUndefined()
    expect(parseDeepLink(""aidev-cli"://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      ""aidev-cli"://open-project?directory=/a",
      ""aidev-cli"://other?directory=/b",
      ""aidev-cli"://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink(""aidev-cli"://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink(""aidev-cli"://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink(""aidev-cli"://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink(""aidev-cli"://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      ""aidev-cli"://new-session?directory=/a",
      ""aidev-cli"://open-project?directory=/b",
      ""aidev-cli"://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __AIDEV_CLI__: {
        deepLinks: [""aidev-cli"://open-project?directory=/a"],
      },
    } as unknown as Window & { __AIDEV_CLI__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual([""aidev-cli"://open-project?directory=/a"])
