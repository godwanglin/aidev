...
import type { SessionNotFoundError } from "@aidev-cli/sdk/v2/client"
...
describe("parseReadableConfigInvalidError", () => {
  test("formats issues with file path", () => {
    ...
    expect(result).toBe(
      ["Arquivo de config em aidev_cli.config.ts invalido: settings.host: Required", "mode: Invalid"].join("\n"),
    )
  })
