  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://aidev_cli.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://aidev_cli.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://aidev_cli.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://aidev_cli.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://aidev_cli.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://aidev_cli.example.com"),
      ),
    ).toEqual({
