...
import { NormalizedProviderListResponse } from "@aidev-cli/session-ui/context"
    ...
    queryFn: () =>
      retry(() =>
        api.list().then((projects) => {
          return projects
            .filter((p) => !!p?.id)
            .filter((p) => !!p.worktree && !p.worktree.includes("aidev-cli-test"))
