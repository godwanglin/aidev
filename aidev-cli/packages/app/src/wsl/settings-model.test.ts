...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when AidevCLI needs attention", () => {
    ...
    expect(
      wslOpencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/aidev-cli",
        version: "1.2.2",
        expectedVersion: "1.2.3",
  ...
  test("does not report ready when AidevCLI is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        aidevCliChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.aidev-cli/bin/aidev-cli",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
            error: "aidev-cli is installed but could not run",
          },
        },
      },
      view: "main",
    ...
    expect(model.primaryButton.action).toBe("install-aidev-cli")
  })
