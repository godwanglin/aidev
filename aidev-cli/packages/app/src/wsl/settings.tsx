    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.aidevCliChecks[item.config.distro]
          const aidevCliAction = () => wslOpencodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-aidev-cli" && wsl.data.job.distro === item.config.distro
          return (
