import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@aidev-cli/client/promise"
import { Button } from "@aidev-cli/ui/button"
import { useDialog } from "@aidev-cli/ui/context/dialog"
import { Dialog } from "@aidev-cli/ui/dialog"
import { Icon } from "@aidev-cli/ui/icon"
import { IconButton } from "@aidev-cli/ui/icon-button"
import { List, type ListRef } from "@aidev-cli/ui/list"
import { ProviderIcon } from "@aidev-cli/ui/provider-icon"
import { Spinner } from "@aidev-cli/ui/spinner"
import { Tag } from "@aidev-cli/ui/tag"
import { TextField } from "@aidev-cli/ui/text-field"
import { ButtonV2 } from "@aidev-cli/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@aidev-cli/ui/v2/dialog-v2"
import { TextInputV2 } from "@aidev-cli/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "aidev-cli-go") return language.t("dialog.provider.aidevCliGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "aidev-cli"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.aidev_cli.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["aidev-cli", "aidev-cli-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "aidev-cli" || provider.id === "aidev-cli-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "aidev-cli"
                                ? "dialog.provider.aidev_cli.tagline"
                                : "dialog.provider.aidevCliGo.tagline",
                            )}
                          </span>
    ...
    if (method.type === "oauth") {
      ...
      await serverSDK()
        ...
        .then((x) => {
          ...
          if (props.provider === "aidev-cli" && platform.platform === "desktop") {
            const url = new URL(x.data.url)
            url.searchParams.set("client_id", "aidev-cli-desktop")
            x.data.url = url.href
          }
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "aidev-cli"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.aidevCliZen.line1")}</div>
              <div>{language.t("provider.connect.aidevCliZen.line2")}</div>
              <div>
                {language.t("provider.connect.aidevCliZen.visit.prefix")}
                <ExternalLink
                  href="https://aidev_cli.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.aidevCliZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.aidevCliZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "aidev-cli"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.aidevCliZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.aidevCliZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.aidevCliZen.visit.prefix")}
                <ExternalLink href="https://aidev_cli.ai/zen" tabIndex={-1}>
                  {language.t("provider.connect.aidevCliZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.aidevCliZen.visit.suffix")}
              </div>
