...
async function configureServers(page: Page, tabs: { type: "session"; server: string; sessionId: string }[] = []) {
  await page.addInitScript(
    ({ serverB, tabs }) => {
      ...
      localStorage.setItem("aidev_cli.global.dat:server", JSON.stringify({ list: [serverB] }))
      localStorage.setItem("aidev_cli.window.browser.dat:tabs", JSON.stringify(tabs))
    },
    { serverB, tabs },
  )
}
