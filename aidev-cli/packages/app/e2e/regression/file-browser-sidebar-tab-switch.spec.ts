...
const directory = "C:/AidevCLI/FileBrowserSidebar"
...
async function setup(
  ...
  await mockAidevCLIServer(page, {
    ...
    provider: {
      ...
      connected: ["aidev-cli"],
      default: { providerID: "aidev-cli", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
  ...
  await page.addInitScript(
    ({ directory, server, sessionID }) => {
      ...
      localStorage.setItem(
        "aidev_cli.global.dat:layout",
        JSON.stringify({ review: { diffStyle: "split", panelOpened: true } }),
      )
      localStorage.setItem(
        "aidev_cli.global.dat:review-panel-v2",
        JSON.stringify({ sidebarOpened: true, sidebarWidth: 240, expandMode: "collapse" }),
      )
      localStorage.setItem(
        "aidev_cli.window.browser.dat:tabs",
