...
import { currentSession, mockAidevCLIServer } from "../utils/mock-server"
import { expectSessionTitle } from "../utils/waits"

const directory = "C:/AidevCLI/SubagentNavigation"
...
test("navigates to a subagent child session missing from the session list", async ({ page }) => {
  ...
  const titlebarRight = page.locator("#aidev-cli-titlebar-right")
  ...
  return [
    {
      info: {
        ...
        agent: "build",
        model: { providerID: "aidev-cli", modelID: "claude-opus-4-6" },
      },
      parts: [
        {
  ...
  await page.addInitScript(
    ({ directory, server, sessionId }) => {
      ...
      localStorage.setItem(
        "aidev_cli.global.dat:server",
        JSON.stringify({
          projects: { local: [{ worktree: directory, expanded: true }] },
          lastProject: { local: directory },
        }),
      )
      localStorage.setItem("aidev_cli.window.browser.dat:tabs", JSON.stringify([{ type: "session", server, sessionId }]))
    },
