...
import { Event } from "@aidev-cli/schema/event"
import { SessionStatusEvent } from "@aidev-cli/schema/session-status-event"
import { SessionV1 } from "@aidev-cli/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/AidevCLI/TimelineStability"
...
export const model = { providerID: "aidev-cli", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("aidev_cli.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["aidev-cli"],
    default: { providerID: "aidev-cli", modelID: "claude-opus-4-6" },
  }
}
