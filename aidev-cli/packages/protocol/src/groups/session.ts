...
import { PromptInput } from "@aidev-cli/schema/prompt-input"
import { Session } from "@aidev-cli/schema/session"
import { Project } from "@aidev-cli/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@aidev-cli/schema/schema"
...
import { SessionEvent } from "@aidev-cli/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this AidevCLI process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this AidevCLI process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
