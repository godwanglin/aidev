...
This is the living design and status document for `@aidev-cli/codemode` and its existing V2 AidevCLI adapter.
...
`packages/aidev-cli/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
