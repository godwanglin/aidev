...
import { createAidev, type ToolPart } from "@aidev-cli/sdk"
...
console.log("🚀 Starting aidev_cli server...")
const aidev_cli = await createAidev({
  port: 0,
})
...
void (async () => {
  const events = await aidev_cli.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new aidev_cli session...")
    const { client, server } = aidev_cli
    ...
    console.log("✅ Created aidev_cli session:", createResult.data.id)
  ...
  console.log("📝 Sending to "aidev-cli":", message.text)
