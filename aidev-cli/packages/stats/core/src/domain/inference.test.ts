...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve aidev_cli route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "aidev-cli")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "aidev-cli")).toBe("openai")
    expect(statProvider("big-pickle", "", "aidev-cli")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "aidev-cli"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep aidev_cli as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "aidev-cli"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "aidev-cli"))).toMatchObject([{ provider: "unknown" }])
    expect(toProviderAggregate(aggregate("muse-spark-1.2-contributor", "unknown"))).toMatchObject([
      { provider: "meta" },
    ])
  })

  test("geo aggregates never keep aidev_cli or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "aidev-cli"), country: "US" })).toMatchObject([
