...
import { define } from "@aidev-cli/plugin/v2/effect"
...
import { LocationServiceMap } from "@aidev-cli/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "aidev_cli.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "aidev_cli.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
