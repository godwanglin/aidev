...
import { Catalog } from "@aidev-cli/core/catalog"
import { Credential } from "@aidev-cli/core/credential"
import { EventV2 } from "@aidev-cli/core/event"
import { Integration } from "@aidev-cli/core/integration"
import { ModelV2 } from "@aidev-cli/core/model"
import { PluginV2 } from "@aidev-cli/core/plugin"
import { PluginHost } from "@aidev-cli/core/plugin/host"
import { OpencodePlugin } from "@aidev-cli/core/plugin/provider/aidev-cli"
import { ProviderV2 } from "@aidev-cli/core/provider"
...
describe("OpencodePlugin", () => {
  it.effect("registers account and service account methods", () =>
    Effect.gen(function* () {
      yield* addPlugin()
      expect((yield* (yield* Integration.Service).get(Integration.ID.make("aidev-cli")))?.methods).toEqual([
        {
          id: Integration.MethodID.make("device"),
          type: "oauth",
          label: "AidevCLI Console account",
        },
  ...
  it.effect("resolves origin-rooted device verification URLs", () =>
    Effect.gen(function* () {
        ...
        Effect.succeed(
          HttpClientResponse.fromWeb(
            ...
            Response.json({
              device_code: "device",
              user_code: "user",
              verification_uri_complete: "/console/device?user_code=user&client_id=aidev_cli-cli",
              expires_in: 60,
              interval: 60,
            }),
          ),
      ...
      const attempt = yield* integration.connection.oauth({
        integrationID: Integration.ID.make("aidev-cli"),
        methodID: Integration.MethodID.make("device"),
        inputs: {},
      })
      expect(attempt.url).toBe("https://aidev_cli.ai/console/device?user_code=user&client_id=aidev_cli-cli")
    }),
  )
  ...
  it.live("loads providers and models from the connected AidevCLI server", () =>
        ...
        Effect.gen(function* () {
          ...
          const provider = required(
            yield* eventually(
              ...
              (item) => item?.integrationID === Integration.ID.make("aidev-cli"),
            ),
          )
          expect(provider).toMatchObject({
            name: "Remote",
            integrationID: "aidev-cli",
            api: {
              type: "aisdk",
  ...
  it.effect("uses a public key and disables paid models without credentials", () =>
    withEnv({ AIDEV_CLI_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
            ...ProviderV2.Info.empty(ProviderV2.ID.make("aidev-cli")),
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.make("aidev-cli"))).request.body.apiKey).toBe("public")
        expect(required(yield* catalog.model.get(ProviderV2.ID.make("aidev-cli"), ModelV2.ID.make("paid"))).enabled).toBe(false)
      }),
    ),
  )
  ...
  it.effect("keeps free models without credentials", () =>
    withEnv({ AIDEV_CLI_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.make("aidev-cli"))).request.body.apiKey).toBe("public")
        expect(required(yield* catalog.model.get(ProviderV2.ID.make("aidev-cli"), ModelV2.ID.make("free"))).enabled).toBe(true)
      }),
    ),
  )
  ...
  it.effect("treats output-only cost as free without credentials", () =>
    withEnv({ AIDEV_CLI_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.make("aidev-cli"))).request.body.apiKey).toBe("public")
        expect(required(yield* catalog.model.get(ProviderV2.ID.make("aidev-cli"), ModelV2.ID.make("output-only"))).enabled).toBe(
          true,
        )
      }),
    ),
  ...
  it.effect("uses AIDEV_CLI_API_KEY as credentials", () =>
    withEnv({ AIDEV_CLI_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.make("aidev-cli"))).request.body.apiKey).toBeUndefined()
        expect(required(yield* catalog.model.get(ProviderV2.ID.make("aidev-cli"), ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )

  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ AIDEV_CLI_API_KEY: undefined, CUSTOM_AIDEV_CLI_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        ...
        yield* integrations.transform((editor) => {
          editor.method.update({
            integrationID: Integration.ID.make("aidev-cli"),
            method: { type: "env", names: ["CUSTOM_AIDEV_CLI_API_KEY"] },
          })
        })
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
  ...
  it.effect("uses configured apiKey as credentials", () =>
    withEnv({ AIDEV_CLI_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.make("aidev-cli"))).request.body.apiKey).toBe("configured")
        expect(required(yield* catalog.model.get(ProviderV2.ID.make("aidev-cli"), ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )

  it.effect("ignores non-aidev-cli providers and models", () =>
    withEnv({ AIDEV_CLI_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
  ...
  it.effect("prefers gpt-5-nano as the aidev_cli small model", () =>
    Effect.gen(function* () {
      const catalog = yield* Catalog.Service
      const providerID = ProviderV2.ID.make("aidev-cli")
