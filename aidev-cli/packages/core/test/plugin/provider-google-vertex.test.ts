...
import { GoogleVertexPlugin } from "@aidev-cli/core/plugin/provider/google-vertex"
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.make("aidev-cli"), (provider) => {
          provider.api = {
            ...
            url: "https://aidev_cli.ai/zen/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.make("aidev-cli")))
