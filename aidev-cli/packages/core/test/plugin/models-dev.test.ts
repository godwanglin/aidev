...
import { ModelsDevPlugin } from "@aidev-cli/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.AIDEV_CLI_MODELS_PATH,
          disabled: Flag.AIDEV_CLI_DISABLE_MODELS_FETCH,
        }
        Flag.AIDEV_CLI_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
        Flag.AIDEV_CLI_DISABLE_MODELS_FETCH = true
        return previous
      }),
      () =>
        Effect.gen(function* () {
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.AIDEV_CLI_MODELS_PATH = previous.path
          Flag.AIDEV_CLI_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
