...
async function tmpdir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "aidev-cli-core-test-"))
  return {
    path: dir,
  ...
  describe("Windows-specific", () => {
    fx.effect(
      ...
      Effect.gen(function* () {
        ...
        const out = yield* ChildProcessSpawner.ChildProcessSpawner.use((svc) =>
          svc.string(
            ChildProcess.make("set", ["AIDEV_CLI_TEST_SHELL"], {
              shell: true,
              extendEnv: true,
              env: { AIDEV_CLI_TEST_SHELL: "ok" },
            }),
          ),
        )
        expect(out).toContain("AIDEV_CLI_TEST_SHELL=ok")
      }),
    )
