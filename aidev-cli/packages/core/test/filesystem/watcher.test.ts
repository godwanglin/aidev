...
import { Watcher } from "@aidev-cli/core/filesystem/watcher"
...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    AIDEV_CLI_EXPERIMENTAL_FILEWATCHER: "true",
    AIDEV_CLI_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
...
function withTmp<A, E, R>(
  ...
  return Effect.acquireRelease(
    Effect.promise(async () => {
      ...
      await $`git config user.email test@aidev_cli.test`.cwd(tmp.path).quiet()
