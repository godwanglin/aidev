...
import { Policy } from "@aidev-cli/core/policy"
...
describe("Config", () => {
  ...
  it.live("loads aidev_cli JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "aidev_cli.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "aidev_cli.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "aidev_cli.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "aidev_cli.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "aidev_cli.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "aidev_cli.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "aidev_cli.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .aidev-cli configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".aidev-cli"), { recursive: true })
            await fs.mkdir(path.join(directory, ".aidev-cli"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "aidev_cli.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "aidev_cli.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "aidev_cli.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "aidev_cli.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "aidev_cli.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".aidev-cli", "aidev_cli.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".aidev-cli", "aidev_cli.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".aidev-cli")),
              AbsolutePath.make(path.join(directory, ".aidev-cli")),
            ])
