...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@aidev-cli/acme")).toBe("@aidev-cli/acme")
    expect(Npm.sanitize("@aidev-cli/acme@1.0.0")).toBe("@aidev-cli/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/aidev-cli/acme.git"
    const expected = win ? "acme@git+https_//github.com/aidev-cli/acme.git" : spec
