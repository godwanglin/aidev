...
import { Permission } from "@aidev-cli/schema/permission"
...
export class Service extends Context.Service<Service, Interface>()("@aidev-cli/v2/Config") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["aidev_cli.json", "aidev_cli.jsonc"]
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            targets: [".aidev-cli", ...names.toReversed()],
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".aidev-cli")
        .toReversed()
        .map((directory) => AbsolutePath.make(directory)),
    ]
    // A config closer to the opened directory should win over one higher up.
    // Search starts nearby, so reverse the results before applying them.
    const directPaths = discovered.filter((item) => path.basename(item) !== ".aidev-cli").toReversed()
    ...
    // global config, project files, then `.aidev-cli` files.
