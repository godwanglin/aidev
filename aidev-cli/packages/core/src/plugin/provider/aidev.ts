...
import type { IntegrationOAuthMethodRegistration } from "@aidev-cli/plugin/v2/effect/integration"
import { define } from "@aidev-cli/plugin/v2/effect/plugin"
import type { CredentialValue } from "@aidev-cli/sdk/v2/types"
...
const defaultServer = "https://aidev_cli.ai/console"
const clientID = "aidev-cli-cli"
...
export const OpencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("OpencodePlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("aidev-cli")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load AidevCLI provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("aidev-cli", (integration) => {
        integration.name = "AidevCLI"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "aidev-cli", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("aidev-cli")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("aidev-cli")
      ...
      const item = catalog.provider.get(ProviderV2.ID.make("aidev-cli"))
      if (!item) return
      const hasKey = Boolean(process.env.AIDEV_CLI_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("aidev-cli")),
