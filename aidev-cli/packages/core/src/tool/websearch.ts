...
import { ToolFailure } from "@aidev-cli/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@aidev-cli/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.AIDEV_CLI_WEBSEARCH_PROVIDER === "exa" || process.env.AIDEV_CLI_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.AIDEV_CLI_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("AIDEV_CLI_EXPERIMENTAL") || truthy("AIDEV_CLI_ENABLE_EXA") || truthy("AIDEV_CLI_EXPERIMENTAL_EXA"),
    enableParallel: truthy("AIDEV_CLI_ENABLE_PARALLEL") || truthy("AIDEV_CLI_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `aidev-cli/${InstallationVersion}`,
