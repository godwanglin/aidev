...
export function fileLogger(file = path.join(Global.Path.log, "aidev_cli.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = process.env.AIDEV_CLI_LOG_LEVEL?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.AIDEV_CLI_PRINT_LOGS === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
