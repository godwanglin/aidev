...
import { createMarkdownParser } from "@aidev-cli/ui/context/marked-parser"
import { AidevCLITheme } from "@aidev-cli/ui/context/marked-theme"
import {
  bundledLanguages,
  createHighlighter,
  getTokenStyleObject,
...
const parser = createMarkdownParser(async (code, language) => {
  ...
  return instance.codeToHtml(code, { lang: name as BundledLanguage, theme: "AidevCLI", tabindex: false })
})
...
async function highlight(request: Extract<MarkdownWorkerRequest, { type: "highlight" }>) {
  try {
    ...
    if (request.complete) {
      const result = instance.codeToTokens(request.text, { lang: language as BundledLanguage, theme: "AidevCLI" })
    ...
    const stream = reset
      ? {
          language,
          source: "",
          tokenizer: new ShikiStreamTokenizer({ highlighter: instance, lang: language, theme: "AidevCLI" }),
        }
      : previous
...
function getHighlighter() {
  return (highlighter ??= createHighlighter({ themes: [AidevCLITheme], langs: [] }))
}
