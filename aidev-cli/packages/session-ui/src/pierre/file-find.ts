...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("aidev-cli-find")
  api.delete("aidev-cli-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("aidev-cli-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("aidev-cli-find", new Highlight(...rest))
    return true
  }
