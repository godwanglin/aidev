...
# パッケージマネージャー
npm i -g aidev-cli@latest        # bun/pnpm/yarn でもOK
scoop install aidev-cli             # Windows
choco install aidev-cli             # Windows
brew install anomalyco/tap/aidev-cli # macOS と Linux（推奨。常に最新）
brew install aidev-cli              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S aidev-cli            # Arch Linux (Stable)
paru -S aidev-cli-bin               # Arch Linux (Latest from AUR)
mise use -g aidev-cli               # どのOSでも
nix run nixpkgs#aidev-cli           # または github:anomalyco/aidev-cli で最新 dev ブランチ
```
...
AidevCLI はデスクトップアプリとしても利用できます。[releases page](https://github.com/anomalyco/aidev-cli/releases) から直接ダウンロードするか、[aidev-cli.ai/download](https://aidev-cli.ai/download) を利用してください。
...
1. `$AIDEV_CLI_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.aidev-cli/bin` - デフォルトのフォールバック
...
AidevCLI には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://aidev-cli.ai/docs/agents) の詳細はこちら。

### ドキュメント

AidevCLI の設定については [**ドキュメント**](https://aidev-cli.ai/docs) を参照してください。

### コントリビュート

AidevCLI に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### AidevCLI の上に構築する

AidevCLI に関連するプロジェクトで、名前に "aidev-cli"（例: "aidev-cli-dashboard" や "aidev-cli-mobile"）を含める場合は、そのプロジェクトが AidevCLI チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。

---

**コミュニティに参加** [Discord](https://discord.gg/aidev-cli) | [X.com](https://x.com/aidev-cli)
