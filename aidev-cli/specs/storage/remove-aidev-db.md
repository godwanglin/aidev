# Remove `packages/aidev-cli/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/aidev-cli/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the aidev-cli legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/aidev-cli/src/storage/db.ts` are concentrated in 22 source files:

- `packages/aidev-cli/src/account/repo.ts`
- `packages/aidev-cli/src/cli/cmd/db.ts`
- `packages/aidev-cli/src/cli/cmd/import.ts`
- `packages/aidev-cli/src/cli/cmd/stats.ts`
- `packages/aidev-cli/src/control-plane/workspace.ts`
- `packages/aidev-cli/src/index.ts`
- `packages/aidev-cli/src/node.ts`
- `packages/aidev-cli/src/permission/index.ts`
- `packages/aidev-cli/src/project/project.ts`
- `packages/aidev-cli/src/server/projectors.ts`
- `packages/aidev-cli/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/aidev-cli/src/server/shared/fence.ts`
- `packages/aidev-cli/src/session/message-v2.ts`
- `packages/aidev-cli/src/session/projectors.ts`
- `packages/aidev-cli/src/session/prompt.ts`
- `packages/aidev-cli/src/session/session.ts`
- `packages/aidev-cli/src/session/todo.ts`
- `packages/aidev-cli/src/share/share-next.ts`
- `packages/aidev-cli/src/storage/db.ts`
- `packages/aidev-cli/src/sync/index.ts`
- `packages/aidev-cli/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy aidev-cli database wrapper; `packages/aidev-cli/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the aidev-cli wrapper.
...
Status: Completed. `SyncEvent` and the aidev-cli projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy aidev-cli database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy aidev-cli database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy aidev-cli database wrapper.
...
All migration groups are complete or superseded. `packages/aidev-cli/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No aidev-cli data-migration group remains.

The previous aidev-cli `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate aidev-cli data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/aidev-cli`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/aidev-cli/src`
- `bun typecheck` from `packages/aidev-cli`
- Relevant package tests from `packages/aidev-cli`, not the repo root
