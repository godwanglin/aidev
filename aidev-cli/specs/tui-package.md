...
Move the canonical AidevCLI terminal application from
`packages/aidev-cli/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/aidev-cli`,
`packages/cli`, or `@aidev-cli/core`.

The SDK is the TUI's AidevCLI boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/aidev-cli`.
- Do not replace private `packages/aidev-cli` imports with `@aidev-cli/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@aidev-cli/tui`.
...
- AidevCLI domain data displayed by the TUI
...
```text
packages/aidev-cli/src/cli/cmd/tui
```

Its private dependency on `packages/aidev-cli` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/aidev-cli/src/*`.
...
Status: Completed. The private `@aidev-cli/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@aidev-cli/tui`.
...
- The package has no dependency on `aidev-cli`, `@aidev-cli/cli`, or
  `@aidev-cli/core`.
...
primitives now live in `@aidev-cli/tui`. The legacy host consumes explicit
...
and build inputs through `@aidev-cli/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads AidevCLI globals or process state;
...
Replace process-global AidevCLI state with resolved TUI inputs.
...
`@aidev-cli/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/aidev-cli` initially.
...
canonical in `@aidev-cli/tui`. Configured references resolve through the new
...
adapters rather than AidevCLI domain implementations.

Make the SDK the only AidevCLI domain boundary used by the TUI.
...
- Domain-facing TUI code imports AidevCLI data and operations only from
  `@aidev-cli/sdk`.
...
`@aidev-cli/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/aidev-cli`.
...
- The package has no imports from `packages/aidev-cli`, `packages/cli`, or
  `@aidev-cli/core`.
...
public `@aidev-cli/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/aidev-cli`.
- Keep the legacy embedded worker and server startup in `packages/aidev-cli`.
- Change those adapters to load config, create transport inputs, and call the
  public `@aidev-cli/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/aidev-cli`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@aidev-cli/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@aidev-cli/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for AidevCLI domain state.
...
cd packages/aidev-cli && bun typecheck
...
rg '@aidev-cli/core|packages/aidev-cli|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/aidev-cli/src packages/aidev-cli/test
```
...
- Build the current-platform `packages/aidev-cli` binary.
