...
Both `action` and `resource` use aidev-cli's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.aidev-cli` files is intentionally deferred until `.aidev-cli` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether aidev-cli permits an operation through its managed execution paths.
