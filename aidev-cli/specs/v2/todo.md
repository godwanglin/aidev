...
The aidev-cli server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@aidev-cli/llm` tool loop after replacing its
...
- plugins get global "aidev-cli" instance like in that post i showed
- aidev-cli instance has stuff like `aidev-cli.session.prompt()` or
  `aidev-cli.tool.register({...})`
...
- decide whether to preserve deprecated `@aidev-cli/llm` orchestration exports
