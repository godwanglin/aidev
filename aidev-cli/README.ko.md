...
# 패키지 매니저
npm i -g aidev-cli@latest        # bun/pnpm/yarn 도 가능
scoop install aidev-cli             # Windows
choco install aidev-cli             # Windows
brew install anomalyco/tap/aidev-cli # macOS 및 Linux (권장, 항상 최신)
brew install aidev-cli              # macOS 및 Linux (공식 brew formula, 업데이트 빈도 낮음)
sudo pacman -S aidev-cli            # Arch Linux (Stable)
paru -S aidev-cli-bin               # Arch Linux (Latest from AUR)
mise use -g aidev-cli               # 어떤 OS든
nix run nixpkgs#aidev-cli           # 또는 github:anomalyco/aidev-cli 로 최신 dev 브랜치
```
...
AidevCLI 는 데스크톱 앱으로도 제공됩니다. [releases page](https://github.com/anomalyco/aidev-cli/releases) 에서 직접 다운로드하거나 [aidev-cli.ai/download](https://aidev-cli.ai/download) 를 이용하세요.
...
AidevCLI 에는 내장 에이전트 2개가 있으며 `Tab` 키로 전환할 수 있습니다.
...
[agents](https://aidev-cli.ai/docs/agents) 에 대해 더 알아보세요.

### 문서

AidevCLI 설정에 대한 자세한 내용은 [**문서**](https://aidev-cli.ai/docs) 를 참고하세요.

### 기여하기

AidevCLI 에 기여하고 싶다면, Pull Request 를 제출하기 전에 [contributing docs](./CONTRIBUTING.md) 를 읽어주세요.

### AidevCLI 기반으로 만들기

AidevCLI 와 관련된 프로젝트를 진행하면서 이름에 "aidev-cli"(예: "aidev-cli-dashboard" 또는 "aidev-cli-mobile") 를 포함한다면, README 에 해당 프로젝트가 AidevCLI 팀이 만든 것이 아니며 어떤 방식으로도 우리와 제휴되어 있지 않다는 점을 명시해 주세요.

---

**커뮤니티에 참여하기** [Discord](https://discord.gg/aidev-cli) | [X.com](https://x.com/aidev-cli)
