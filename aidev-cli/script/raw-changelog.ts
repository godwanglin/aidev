...
const repo = process.env.GH_REPO ?? "anomalyco/aidev-cli"
const bot = ["actions-user", "github-actions[bot]", "aidev-cli", "aidev-cli-agent[bot]"]
const team = [
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/aidev-cli packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/aidev-cli/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/aidev-cli/")) areas.add("core")
