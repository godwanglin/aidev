...
const model = "aidev-cli/deepseek-v4-flash"
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with aidev-cli...`)
  ...
  try {
    await $`aidev-cli run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  aidev-cli failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with aidev-cli")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with aidev-cli...")
