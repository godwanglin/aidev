...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "aidev-cli/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".aidev-cli/glossary/zh-cn.md"
  if (locale === "zht") return ".aidev-cli/glossary/zh-tw.md"
  return `.aidev-cli/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("AidevCLI did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("AidevCLI returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("AidevCLI session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel AidevCLI runs for 'all' (default: 4)
      --model <provider/id>  AidevCLI model (default: aidev-cli/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running AidevCLI
...
`)
  ...
  if (failed.length) console.error(`\nAidevCLI failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.AIDEV_CLI_DISABLE_PROJECT_CONFIG = "1"
  env.AIDEV_CLI_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["aidev-cli", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.AIDEV_CLI_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["aidev-cli", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.AIDEV_CLI_CONFIG
  delete env.AIDEV_CLI_CONFIG_DIR
  delete env.AIDEV_CLI_CONFIG_CONTENT
  delete env.AIDEV_CLI_PERMISSION
  delete env.AIDEV_CLI_AUTO_SHARE
  return env
}
