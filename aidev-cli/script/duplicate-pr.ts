...
import { createOpencode } from "@aidev-cli/sdk"
...
`)
  ...
  const aidev-cli = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await aidev-cli.client.session.create()
    const result = await aidev-cli.client.session
      .prompt({
